...
/**
* @brief Encapsulates a complete Vulkan framebuffer with an arbitrary number and
...
public:
  ...
  */
  ...
  /**
  * Destroy and free Vulkan resources used for the framebuffer and all of it's
  * attachments
  */
  ~Framebuffer() {
    assert(vulkanDevice);
  ...
  /**
  * Add a new attachment described by createinfo to the framebuffer's attachment
  * list
  *
  ...
  */
  uint32_t addAttachment(vks::AttachmentCreateInfo createinfo) {
    ...
    VK_CHECK_RESULT(vkCreateImage(vulkanDevice->logicalDevice, &image, nullptr,
                                  &attachment.image));
    vkGetImageMemoryRequirements(vulkanDevice->logicalDevice, attachment.image,
                                 &memReqs);
    ...
    VK_CHECK_RESULT(vkAllocateMemory(vulkanDevice->logicalDevice, &memAlloc,
                                     nullptr, &attachment.memory));
    VK_CHECK_RESULT(vkBindImageMemory(vulkanDevice->logicalDevice,
                                      attachment.image, attachment.memory, 0));
    ...
    VK_CHECK_RESULT(vkCreateImageView(vulkanDevice->logicalDevice, &imageView,
  ...
  */
  VkResult createSampler(VkFilter magFilter, VkFilter minFilter,
    ...
    return vkCreateSampler(vulkanDevice->logicalDevice, &samplerInfo, nullptr,
                           &sampler);
  }
  ...
  */
  VkResult createRenderPass() {
    ...
    VK_CHECK_RESULT(vkCreateFramebuffer(
        vulkanDevice->logicalDevice, &framebufferInfo, nullptr, &framebuffer));
