...
VkCommandBuffer
VulkanExampleBase::createCommandBuffer(VkCommandBufferLevel level, bool begin) {
...
VulkanExampleBase::loadShader(std::string fileName,
...
bool VulkanExampleBase::initVulkan() {
  ...
  if (settings.validation) {
    ...
    // displayed
    // For validating (debugging) an appplication the error and warning bits
    // should suffice
      ...
      if (gpuCount == 0) {
        ...
        for (uint32_t i = 0; i < gpuCount; i++) {
          ...
          std::cout << "Device [" << i << "] : " << deviceProperties.deviceName
                    << std::endl;
          std::cout << " Type: " << vks::tools::physicalDeviceTypeString(
  ...
  // overlay have been sumbitted and executed
  // Will be inserted after the render complete semaphore if the UI overlay is
  // enabled
  VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr,
...
void VulkanExampleBase::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam,
                                       LPARAM lParam) {
  switch (uMsg) {
  case WM_CLOSE:
    prepared = false;
...
/*static*/ void
VulkanExampleBase::pointerEnterCb(void *data, wl_pointer *pointer,
...
/*static*/ void
VulkanExampleBase::pointerButtonCb(void *data, wl_pointer *pointer,
...
/*static*/ void
VulkanExampleBase::keyboardKeymapCb(void *data, struct wl_keyboard *keyboard,
...
/*static*/ void
VulkanExampleBase::keyboardModifiersCb(void *data, struct wl_keyboard *keyboard,
