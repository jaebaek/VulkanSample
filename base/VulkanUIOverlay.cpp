...
UIOverlay::UIOverlay(vks::UIOverlayCreateInfo createInfo) {
  ...
  io.DisplaySize =
      ImVec2((float)(createInfo.width), (float)(createInfo.height));
...
UIOverlay::~UIOverlay() {
  ...
  vkDestroyDescriptorSetLayout(createInfo.device->logicalDevice,
                               descriptorSetLayout, nullptr);
  vkDestroyDescriptorPool(createInfo.device->logicalDevice, descriptorPool,
                          nullptr);
  vkDestroyPipelineLayout(createInfo.device->logicalDevice, pipelineLayout,
                          nullptr);
  vkDestroyPipelineCache(createInfo.device->logicalDevice, pipelineCache,
                         nullptr);
  ...
  vkFreeCommandBuffers(createInfo.device->logicalDevice, commandPool,
...
void UIOverlay::prepareResources() {
  ...
  VK_CHECK_RESULT(vkCreateImage(createInfo.device->logicalDevice, &imageInfo,
                                nullptr, &fontImage));
  VkMemoryRequirements memReqs;
  vkGetImageMemoryRequirements(createInfo.device->logicalDevice, fontImage,
                               &memReqs);
  VkMemoryAllocateInfo memAllocInfo = vks::initializers::memoryAllocateInfo();
  memAllocInfo.allocationSize = memReqs.size;
  memAllocInfo.memoryTypeIndex = createInfo.device->getMemoryType(
      memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
  VK_CHECK_RESULT(vkAllocateMemory(createInfo.device->logicalDevice,
                                   &memAllocInfo, nullptr, &fontMemory));
  VK_CHECK_RESULT(vkBindImageMemory(createInfo.device->logicalDevice, fontImage,
                                    fontMemory, 0));
  ...
  VK_CHECK_RESULT(vkCreateImageView(createInfo.device->logicalDevice, &viewInfo,
                                    nullptr, &fontView));
  ...
  VK_CHECK_RESULT(
      createInfo.device->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
  ...
  VkCommandBuffer copyCmd = createInfo.device->createCommandBuffer(
  ...
  VK_CHECK_RESULT(vkCreateSampler(createInfo.device->logicalDevice,
  ...
  VK_CHECK_RESULT(vkCreateCommandPool(createInfo.device->logicalDevice,
  ...
  VkCommandBufferAllocateInfo cmdBufAllocateInfo =
      vks::initializers::commandBufferAllocateInfo(
          ...
          static_cast<uint32_t>(cmdBuffers.size()));
  VK_CHECK_RESULT(vkAllocateCommandBuffers(createInfo.device->logicalDevice,
                                           &cmdBufAllocateInfo,
                                           cmdBuffers.data()));
  ...
  VK_CHECK_RESULT(vkAllocateDescriptorSets(createInfo.device->logicalDevice,
  ...
  VK_CHECK_RESULT(vkCreateFence(createInfo.device->logicalDevice,
...
void UIOverlay::preparePipeline() {
  ...
  pipelineCreateInfo.stageCount =
      static_cast<uint32_t>(createInfo.shaders.size());
  ...
  VK_CHECK_RESULT(
      vkCreateGraphicsPipelines(createInfo.device->logicalDevice, pipelineCache,
...
void UIOverlay::prepareRenderPass() {
  ...
  VK_CHECK_RESULT(vkCreateRenderPass(createInfo.device->logicalDevice,
...
void UIOverlay::updateCommandBuffers() {
  ...
  renderPassBeginInfo.clearValueCount =
      static_cast<uint32_t>(createInfo.clearValues.size());
  ...
  for (size_t i = 0; i < cmdBuffers.size(); ++i) {
    ...
    if (vks::debugmarker::active) {
      vks::debugmarker::beginRegion(cmdBuffers[i], "UI overlay",
    ...
    vkCmdBindIndexBuffer(cmdBuffers[i], indexBuffer.buffer, 0,
    ...
    VkRect2D scissor =
        vks::initializers::rect2D((int32_t)ImGui::GetIO().DisplaySize.x,
                                  (int32_t)ImGui::GetIO().DisplaySize.y, 0, 0);
    ...
    vkCmdPushConstants(cmdBuffers[i], pipelineLayout,
    ...
    for (int32_t j = 0; j < imDrawData->CmdListsCount; j++) {
      ...
      for (int32_t k = 0; k < cmd_list->CmdBuffer.Size; k++) {
        ...
        vkCmdDrawIndexed(cmdBuffers[i], pcmd->ElemCount, 1, indexOffset,
                         vertexOffset, 0);
...
void UIOverlay::submit(VkQueue queue, uint32_t bufferindex,
  ...
  VK_CHECK_RESULT(vkWaitForFences(createInfo.device->logicalDevice, 1, &fence,
                                  VK_TRUE, UINT64_MAX));
...
bool UIOverlay::button(const char *caption) { return ImGui::Button(caption); }
