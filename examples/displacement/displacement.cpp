...
public:
  ...
  void loadAssets() {
    models.object.loadFromFile(getAssetPath() + "models/plane.obj",
                               vertexLayout, 0.25f, vulkanDevice, queue);
    // Textures
    if (vulkanDevice->features.textureCompressionBC) {
      textures.colorHeightMap.loadFromFile(
          getAssetPath() + "textures/stonefloor03_color_bc3_unorm.ktx",
          VK_FORMAT_BC3_UNORM_BLOCK, vulkanDevice, queue);
    } else if (vulkanDevice->features.textureCompressionASTC_LDR) {
      textures.colorHeightMap.loadFromFile(
          getAssetPath() + "textures/stonefloor03_color_astc_8x8_unorm.ktx",
          VK_FORMAT_ASTC_8x8_UNORM_BLOCK, vulkanDevice, queue);
    } else if (vulkanDevice->features.textureCompressionETC2) {
      textures.colorHeightMap.loadFromFile(
          getAssetPath() + "textures/stonefloor03_color_etc2_unorm.ktx",
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      VkRect2D scissor = vks::initializers::rect2D(
          splitScreen ? width / 2 : width, height, 0, 0);
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorImageInfo texDescriptor =
        vks::initializers::descriptorImageInfo(textures.colorHeightMap.sampler,
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/displacement/base.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/displacement/base.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    shaderStages[2] = loadShader(
        getAssetPath() + "shaders/displacement/displacement.tesc.spv",
        VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
    shaderStages[3] = loadShader(
        getAssetPath() + "shaders/displacement/displacement.tese.spv",
        VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
  ...
  void updateUniformBuffers() {
    ...
    uboTessEval.model =
        viewMatrix * glm::translate(uboTessEval.model, glm::vec3(0, 0, 0));
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      ...
      if (overlay->inputFloat("Strength", &uboTessEval.tessStrength, 0.025f,
                              3)) {
        updateUniformBuffers();
      }
