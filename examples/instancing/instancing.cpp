/*
* Vulkan Example - Instanced mesh rendering, uses a separate vertex buffer for
* instanced data
*
...
public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      // Render instances
      vkCmdDrawIndexed(drawCmdBuffers[i], models.rock.indexCount,
  ...
  void loadAssets() {
    models.rock.loadFromFile(getAssetPath() + "models/rock01.dae", vertexLayout,
                             0.1f, vulkanDevice, queue);
    models.planet.loadFromFile(getAssetPath() + "models/sphere.obj",
                               vertexLayout, 0.2f, vulkanDevice, queue);
    ...
    textures.rocks.loadFromFile(getAssetPath() + "textures/texturearray_rocks" +
                                    texFormatSuffix + ".ktx",
                                texFormat, vulkanDevice, queue);
    textures.planet.loadFromFile(getAssetPath() + "textures/lavaplanet" +
                                     texFormatSuffix + ".ktx",
                                 texFormat, vulkanDevice, queue);
  }
  ...
  void setupDescriptorSet() {
    ...
    descripotrSetAllocInfo = vks::initializers::descriptorSetAllocateInfo(
    ...
    writeDescriptorSets = {
        vks::initializers::writeDescriptorSet(
            descriptorSets.instancedRocks, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
    ...
    // Planet
    VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &descripotrSetAllocInfo,
                                             &descriptorSets.planet));
    writeDescriptorSets = {
        vks::initializers::writeDescriptorSet(
            descriptorSets.planet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
            &uniformBuffers.scene
                 .descriptor), // Binding 0 : Vertex shader uniform buffer
        vks::initializers::writeDescriptorSet(
            descriptorSets.planet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
  ...
  void preparePipelines() {
    ...
    // This example uses two different input states, one for the instanced part
    // and one for non-instanced rendering
    ...
    bindingDescriptions = {
        ...
        vks::initializers::vertexInputBindingDescription(
            INSTANCE_BUFFER_BIND_ID, sizeof(InstanceData),
            VK_VERTEX_INPUT_RATE_INSTANCE)};

    // Vertex attribute bindings
    // Note that the shader declaration for per-vertex and per-instance
    // attributes is the same, the different input rates are only stored in the
    // bindings:
    // instanced.vert:
    ...
    attributeDescriptions = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            INSTANCE_BUFFER_BIND_ID, 4, VK_FORMAT_R32G32B32_SFLOAT,
            0), // Location 4: Position
        vks::initializers::vertexInputAttributeDescription(
            INSTANCE_BUFFER_BIND_ID, 5, VK_FORMAT_R32G32B32_SFLOAT,
            sizeof(float) * 3), // Location 5: Rotation
        vks::initializers::vertexInputAttributeDescription(
            INSTANCE_BUFFER_BIND_ID, 6, VK_FORMAT_R32_SFLOAT,
            sizeof(float) * 6), // Location 6: Scale
        vks::initializers::vertexInputAttributeDescription(
            INSTANCE_BUFFER_BIND_ID, 7, VK_FORMAT_R32_SINT,
    ...
    // Instancing pipeline
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/instancing/instancing.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/instancing/instancing.frag.spv",
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/instancing/planet.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/instancing/planet.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/instancing/starfield.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/instancing/starfield.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareInstanceData() {
    ...
    std::uniform_int_distribution<uint32_t> rndTextureIndex(
        0, textures.rocks.layerCount);
    ...
    for (auto i = 0; i < INSTANCE_COUNT / 2; i++) {
      ...
      // Inner ring
      rho = sqrt((pow(ring0[1], 2.0f) - pow(ring0[0], 2.0f)) *
                     uniformDist(rndGenerator) +
                 pow(ring0[0], 2.0f));
      theta = 2.0 * M_PI * uniformDist(rndGenerator);
      instanceData[i].pos =
          glm::vec3(rho * cos(theta), uniformDist(rndGenerator) * 0.5f - 0.25f,
                    rho * sin(theta));
      instanceData[i].rot = glm::vec3(M_PI * uniformDist(rndGenerator),
                                      M_PI * uniformDist(rndGenerator),
                                      M_PI * uniformDist(rndGenerator));
      instanceData[i].scale =
          1.5f + uniformDist(rndGenerator) - uniformDist(rndGenerator);
      instanceData[i].texIndex = rndTextureIndex(rndGenerator);
      instanceData[i].scale *= 0.75f;

      // Outer ring
      rho = sqrt((pow(ring1[1], 2.0f) - pow(ring1[0], 2.0f)) *
                     uniformDist(rndGenerator) +
                 pow(ring1[0], 2.0f));
      theta = 2.0 * M_PI * uniformDist(rndGenerator);
      ...
      instanceData[i + INSTANCE_COUNT / 2].rot = glm::vec3(
          M_PI * uniformDist(rndGenerator), M_PI * uniformDist(rndGenerator),
          M_PI * uniformDist(rndGenerator));
      instanceData[i + INSTANCE_COUNT / 2].scale =
          1.5f + uniformDist(rndGenerator) - uniformDist(rndGenerator);
      instanceData[i + INSTANCE_COUNT / 2].texIndex =
          rndTextureIndex(rndGenerator);
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                   ...
                                   instanceBuffer.size, &stagingBuffer.buffer,
                                   &stagingBuffer.memory, instanceData.data()));

    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, instanceBuffer.size,
    ...
    vkCmdCopyBuffer(copyCmd, stagingBuffer.buffer, instanceBuffer.buffer, 1,
                    &copyRegion);
  ...
  void updateUniformBuffer(bool viewChanged) {
    if (viewChanged) {
      ...
      uboVS.view = glm::translate(glm::mat4(1.0f),
                                  cameraPos + glm::vec3(0.0f, 0.0f, zoom));
