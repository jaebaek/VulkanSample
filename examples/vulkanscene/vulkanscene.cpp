...
public:
  ...
  void loadAssets() {
    // Models
    std::vector<std::string> modelFiles = {"vulkanscenelogos.dae",
                                           "vulkanscenebackground.dae",
                                           "vulkanscenemodels.dae", "cube.obj"};
    std::vector<VkPipeline *> modelPipelines = {
        &pipelines.logos, &pipelines.models, &pipelines.models,
        &pipelines.skybox};
    for (auto i = 0; i < modelFiles.size(); i++) {
      DemoModel model;
      model.pipeline = modelPipelines[i];
      vks::ModelCreateInfo modelCreateInfo(glm::vec3(1.0f), glm::vec3(1.0f),
                                           glm::vec3(0.0f));
      if (modelFiles[i] != "cube.obj") {
        modelCreateInfo.center.y += 1.15f;
      }
      model.model.loadFromFile(getAssetPath() + "models/" + modelFiles[i],
                               vertexLayout, &modelCreateInfo, vulkanDevice,
                               queue);
      demoModels.push_back(model);
    }
    // Textures
    textures.skybox.loadFromFile(getAssetPath() + "textures/cubemap_vulkan.ktx",
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorImageInfo texDescriptorCubeMap =
        vks::initializers::descriptorImageInfo(
            textures.skybox.sampler, textures.skybox.view,
  ...
  void preparePipelines() {
    ...
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            ...
            sizeof(float) * 5), // Location 2: Texture coordinates
        vks::initializers::vertexInputAttributeDescription(
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/vulkanscene/mesh.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/vulkanscene/mesh.frag.spv",
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/vulkanscene/logo.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/vulkanscene/logo.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                              &pipelineCreateInfo, nullptr,
                                              &pipelines.logos));
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/vulkanscene/skybox.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/vulkanscene/skybox.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                              &pipelineCreateInfo, nullptr,
                                              &pipelines.skybox));
  ...
  void prepareUniformBuffers() {
    vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                               ...
                               &uniformData.meshVS, sizeof(uboVS));
  ...
  void updateUniformBuffers() {
    ...
    uboVS.view =
        glm::lookAt(glm::vec3(0, 0, -zoom), cameraPos, glm::vec3(0, 1, 0));
