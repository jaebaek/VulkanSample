...
public:
  ...
  struct {
    vks::Buffer storageBuffer; // (Shader) storage buffer object containing the
                               // particles
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &compute.storageBuffer.buffer, offsets);
  ...
  void buildComputeCommandBuffer() {
    ...
    bufferBarrier.srcAccessMask =
        VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT; // Vertex shader invocations have
                                             // finished reading from the buffer
    bufferBarrier.dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT; // Compute shader
                                                              // wants to write
                                                              // to the buffer
    ...
    bufferBarrier.srcQueueFamilyIndex =
        vulkanDevice->queueFamilyIndices.graphics; // Required as compute and
                                                   // graphics queue may have
                                                   // different families
    bufferBarrier.dstQueueFamilyIndex =
        vulkanDevice->queueFamilyIndices.compute; // Required as compute and
                                                  // graphics queue may have
                                                  // different families
    ...
    // Add memory barrier to ensure that compute shader has finished writing to
    // the buffer
    // Without this the (rendering) vertex shader may display incomplete results
    // (partial data from last frame)
    bufferBarrier.srcAccessMask =
        VK_ACCESS_SHADER_WRITE_BIT; // Compute shader has finished writes to the
                                    // buffer
    bufferBarrier.dstAccessMask =
        VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT; // Vertex shader invocations want
    ...
    vkCmdPipelineBarrier(compute.commandBuffer,
                         ...
                         VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_FLAGS_NONE, 0,
                         nullptr, 1, &bufferBarrier, 0, nullptr);
  ...
  void prepareStorageBuffers() {
    ...
    vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &compute.storageBuffer,
        storageBufferSize);
    ...
    vkCmdCopyBuffer(copyCmd, stagingBuffer.buffer, compute.storageBuffer.buffer,
                    1, &copyRegion);
    ...
    vertices.attributeDescriptions[0] =
        vks::initializers::vertexInputAttributeDescription(
            VERTEX_BUFFER_BIND_ID, 0, VK_FORMAT_R32G32_SFLOAT,
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/computeparticles/particle.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/computeparticles/particle.frag.spv",
  ...
  void prepareCompute() {
    ...
    // queue and prefers queue families that only support compute
    // Depending on the implementation this may result in different queue family
    // indices for graphics and computes,
    ...
    std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
        ...
        vks::initializers::writeDescriptorSet(
            compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 0,
    ...
    computePipelineCreateInfo.stage = loadShader(
        getAssetPath() + "shaders/computeparticles/particle.comp.spv",
  ...
  void updateUniformBuffers() {
    ...
    if (animate) {
      ...
      float normalizedMx = (mousePos.x - static_cast<float>(width / 2)) /
                           static_cast<float>(width / 2);
      float normalizedMy = (mousePos.y - static_cast<float>(height / 2)) /
