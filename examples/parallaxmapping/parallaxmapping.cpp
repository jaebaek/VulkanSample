...
public:
  ...
  vks::VertexLayout vertexLayout = vks::VertexLayout({
      ...
      vks::VERTEX_COMPONENT_NORMAL, vks::VERTEX_COMPONENT_TANGENT,
  ...
  void loadAssets() {
    models.quad.loadFromFile(getAssetPath() + "models/plane_z.obj",
                             vertexLayout, 0.1f, vulkanDevice, queue);

    // Textures
    textures.normalHeightMap.loadFromFile(
    ...
    if (vulkanDevice->features.textureCompressionBC) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "textures/rocks_color_bc3_unorm.dds",
          VK_FORMAT_BC3_UNORM_BLOCK, vulkanDevice, queue);
    } else if (vulkanDevice->features.textureCompressionASTC_LDR) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "textures/rocks_color_astc_8x8_unorm.ktx",
          VK_FORMAT_ASTC_8x8_UNORM_BLOCK, vulkanDevice, queue);
    } else if (vulkanDevice->features.textureCompressionETC2) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "textures/rocks_color_etc2_unorm.ktx",
  ...
  void setupDescriptorSetLayout() {
    std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
        ...
        vks::initializers::descriptorSetLayoutBinding(
            ...
            1), // Binding 1: Fragment shader color map image sampler
        vks::initializers::descriptorSetLayoutBinding(
            VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
            VK_SHADER_STAGE_FRAGMENT_BIT,
            2), // Binding 2: Fragment combined normal and heightmap
        vks::initializers::descriptorSetLayoutBinding(
            VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT,
            3), // Binding 3: Fragment shader uniform buffer
    };
  ...
  void setupDescriptorSet() {
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        ...
        vks::initializers::writeDescriptorSet(
            ...
            &textures.colorMap
                 .descriptor), // Binding 1: Fragment shader image sampler
        vks::initializers::writeDescriptorSet(
            descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2,
            &textures.normalHeightMap
                 .descriptor), // Binding 2: Combined normal and heightmap
        vks::initializers::writeDescriptorSet(
            descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 3,
            &uniformBuffers.fragmentShader
                 .descriptor), // Binding 3: Fragment shader uniform buffer
    };
  ...
  void preparePipelines() {
    ...
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            ...
            sizeof(float) * 3), // Location 1: Texture coordinates
        vks::initializers::vertexInputAttributeDescription(
        ...
        vks::initializers::vertexInputAttributeDescription(
            ...
            sizeof(float) * 11), // Location 4: Bitangent
    };
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/parallaxmapping/parallax.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/parallaxmapping/parallax.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        &uniformBuffers.vertexShader, sizeof(ubos.vertexShader)));
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.fragmentShader, sizeof(ubos.fragmentShader)));
  ...
  void updateUniformBuffers() {
    ...
    ubos.vertexShader.model = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f),
                                          glm::vec3(1.0f, 0.0f, 0.0f));
    ;
    ubos.vertexShader.model =
        glm::rotate(ubos.vertexShader.model, glm::radians(180.0f),
    ...
    memcpy(uniformBuffers.vertexShader.mapped, &ubos.vertexShader,
           sizeof(ubos.vertexShader));

    // Fragment shader
    memcpy(uniformBuffers.fragmentShader.mapped, &ubos.fragmentShader,
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      if (overlay->comboBox("Mode", &ubos.fragmentShader.mappingMode,
                            mappingModes)) {
        updateUniformBuffers();
      }
    }
