...
public:
  ...
  void prepareCubeMap() {
    ...
    VK_CHECK_RESULT(
        vkCreateImage(device, &imageCreateInfo, nullptr, &shadowCubeMap.image));
    ...
    VK_CHECK_RESULT(vkAllocateMemory(device, &memAllocInfo, nullptr,
                                     &shadowCubeMap.deviceMemory));
    VK_CHECK_RESULT(vkBindImageMemory(device, shadowCubeMap.image,
                                      shadowCubeMap.deviceMemory, 0));
    ...
    vks::tools::setImageLayout(
        layoutCmd, shadowCubeMap.image, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    VK_CHECK_RESULT(
        vkCreateSampler(device, &sampler, nullptr, &shadowCubeMap.sampler));
    ...
    VK_CHECK_RESULT(
        vkCreateImageView(device, &view, nullptr, &shadowCubeMap.view));
  }
  ...
  void prepareOffscreenFramebuffer() {
    ...
    vks::tools::setImageLayout(
        layoutCmd, offscreenPass.color.image, VK_IMAGE_ASPECT_COLOR_BIT,
    ...
    vks::tools::setImageLayout(
        layoutCmd, offscreenPass.depth.image,
        VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT,
  ...
  void updateCubeFace(uint32_t faceIndex) {
    ...
    case 0: // POSITIVE_X
      ...
      viewMatrix = glm::rotate(viewMatrix, glm::radians(180.0f),
    ...
    vkCmdPushConstants(offscreenPass.commandBuffer, pipelineLayouts.offscreen,
    ...
    vkCmdBindVertexBuffers(offscreenPass.commandBuffer, VERTEX_BUFFER_BIND_ID,
                           1, &models.scene.vertices.buffer, offsets);
    vkCmdBindIndexBuffer(offscreenPass.commandBuffer,
                         models.scene.indices.buffer, 0, VK_INDEX_TYPE_UINT32);
    vkCmdDrawIndexed(offscreenPass.commandBuffer, models.scene.indexCount, 1, 0,
                     0, 0);

    vkCmdEndRenderPass(offscreenPass.commandBuffer);
    // Make sure color writes to the framebuffer are finished before using it as
    // transfer source
    vks::tools::setImageLayout(
        offscreenPass.commandBuffer, offscreenPass.color.image,
    ...
    vks::tools::setImageLayout(offscreenPass.commandBuffer, shadowCubeMap.image,
    ...
    vkCmdCopyImage(offscreenPass.commandBuffer, offscreenPass.color.image,
                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, shadowCubeMap.image,
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      if (displayCubeMap) {
        ...
        vkCmdDrawIndexed(drawCmdBuffers[i], models.skybox.indexCount, 1, 0, 0,
                         0);
      } else {
        vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipelines.scene);
  ...
  void loadAssets() {
    models.skybox.loadFromFile(getAssetPath() + "models/cube.obj", vertexLayout,
                               2.0f, vulkanDevice, queue);
    models.scene.loadFromFile(getAssetPath() + "models/shadowscene_fire.dae",
  ...
  void setupDescriptorSets() {
    ...
    VkDescriptorImageInfo texDescriptor =
        vks::initializers::descriptorImageInfo(
            shadowCubeMap.sampler, shadowCubeMap.view,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    std::vector<VkWriteDescriptorSet> sceneDescriptorSets = {
        ...
        vks::initializers::writeDescriptorSet(
            ...
            &texDescriptor)};
    vkUpdateDescriptorSets(device, sceneDescriptorSets.size(),
                           sceneDescriptorSets.data(), 0, NULL);
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/shadowmapomni/scene.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/shadowmapomni/scene.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);

    VkGraphicsPipelineCreateInfo pipelineCreateInfo =
        vks::initializers::pipelineCreateInfo(pipelineLayouts.scene, renderPass,
                                              0);
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/shadowmapomni/cubemapdisplay.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/shadowmapomni/cubemapdisplay.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    // Offscreen pipeline
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/shadowmapomni/offscreen.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/shadowmapomni/offscreen.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void updateUniformBuffers() {
    ...
    uboVSscene.view = glm::translate(
        glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, displayCubeMap ? 0.0f : zoom));

    uboVSscene.model = glm::mat4(1.0f);
    uboVSscene.model = glm::rotate(uboVSscene.model, glm::radians(rotation.x),
                                   glm::vec3(1.0f, 0.0f, 0.0f));
    uboVSscene.model = glm::rotate(uboVSscene.model, glm::radians(rotation.y),
                                   glm::vec3(0.0f, 1.0f, 0.0f));
    uboVSscene.model = glm::rotate(uboVSscene.model, glm::radians(rotation.z),
                                   glm::vec3(0.0f, 0.0f, 1.0f));
  ...
  void updateUniformBufferOffscreen() {
    ...
    uboOffscreenVS.projection =
        glm::perspective((float)(M_PI / 2.0), 1.0f, zNear, zFar);

    uboOffscreenVS.view = glm::mat4(1.0f);
    uboOffscreenVS.model = glm::translate(
        glm::mat4(1.0f), glm::vec3(-lightPos.x, -lightPos.y, -lightPos.z));
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      if (overlay->checkBox("Display shadow cube render target",
                            &displayCubeMap)) {
        buildCommandBuffers();
      }
    }
