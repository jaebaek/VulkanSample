...
*
* Summary:
* Implements a deferred rendering setup with a forward transparency pass using
* sub passes
*
* Sub passes allow reading from the previous framebuffer (in the same render
* pass) at
* the same pixel position.
*
* This is a feature that was especially designed for tile-based-renderers
* (mostly mobile GPUs) and is a new optomization feature in Vulkan for those GPU
* types.
*
*/
...
public:
  ...
  vks::VertexLayout vertexLayout = vks::VertexLayout({
      vks::VERTEX_COMPONENT_POSITION, vks::VERTEX_COMPONENT_COLOR,
      vks::VERTEX_COMPONENT_NORMAL, vks::VERTEX_COMPONENT_UV,
  });
  ...
  ~VulkanExample() {
    ...
    vkDestroyDescriptorSetLayout(device, descriptorSetLayouts.composition,
                                 nullptr);
    vkDestroyDescriptorSetLayout(device, descriptorSetLayouts.transparent,
                                 nullptr);
  ...
  void createGBufferAttachments() {
    createAttachment(VK_FORMAT_R16G16B16A16_SFLOAT,
                     ...
                     &attachments.position); // (World space) Positions
    createAttachment(VK_FORMAT_R16G16B16A16_SFLOAT,
                     VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                     &attachments.normal); // (World space) Normals
    createAttachment(VK_FORMAT_R8G8B8A8_UNORM,
                     VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                     &attachments.albedo); // Albedo (color)
  }
  ...
  void setupRenderPass() {
    ...
    // Use the color/depth attachments filled in the first pass as input
    // attachments
    ...
    VK_CHECK_RESULT(
        vkCreateRenderPass(device, &renderPassInfo, nullptr, &uiRenderPass));
  }
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      {
        vks::debugmarker::beginRegion(drawCmdBuffers[i],
                                      "Subpass 0: Deferred G-Buffer creation",
        ...
        vkCmdBindDescriptorSets(
            ...
            pipelineLayouts.offscreen, 0, 1, &descriptorSets.scene, 0, NULL);
        vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                               &models.scene.vertices.buffer, offsets);
        vkCmdBindIndexBuffer(drawCmdBuffers[i], models.scene.indices.buffer, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(drawCmdBuffers[i], models.scene.indexCount, 1, 0, 0,
                         0);
      ...
      // Second sub pass
      // This subpass will use the G-Buffer components that have been filled in
      // the first subpass as input attachment for the final compositing
      {
        vks::debugmarker::beginRegion(drawCmdBuffers[i],
                                      "Subpass 1: Deferred composition",
                                      glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
      ...
      // Third subpass
      // Render transparent geometry using a forward pass that compares against
      // depth generted during G-Buffer fill
      {
        vks::debugmarker::beginRegion(drawCmdBuffers[i],
                                      "Subpass 2: Forward transparency",
                                      glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));
        ...
        vkCmdBindDescriptorSets(drawCmdBuffers[i],
                                VK_PIPELINE_BIND_POINT_GRAPHICS,
                                pipelineLayouts.transparent, 0, 1,
                                &descriptorSets.transparent, 0, NULL);
        vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                               &models.transparent.vertices.buffer, offsets);
        vkCmdBindIndexBuffer(drawCmdBuffers[i],
                             models.transparent.indices.buffer, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(drawCmdBuffers[i], models.transparent.indexCount, 1, 0,
                         0, 0);
  ...
  void loadAssets() {
    models.scene.loadFromFile(getAssetPath() + "models/samplebuilding.dae",
                              vertexLayout, 1.0f, vulkanDevice, queue);
    models.transparent.loadFromFile(getAssetPath() +
                                        "models/samplebuilding_glass.dae",
                                    vertexLayout, 1.0f, vulkanDevice, queue);
    // Textures
    if (vulkanDevice->features.textureCompressionBC) {
      textures.glass.loadFromFile(
          getAssetPath() + "textures/colored_glass_bc3_unorm.ktx",
          VK_FORMAT_BC3_UNORM_BLOCK, vulkanDevice, queue);
    } else if (vulkanDevice->features.textureCompressionASTC_LDR) {
      textures.glass.loadFromFile(
          getAssetPath() + "textures/colored_glass_astc_8x8_unorm.ktx",
          VK_FORMAT_ASTC_8x8_UNORM_BLOCK, vulkanDevice, queue);
    } else if (vulkanDevice->features.textureCompressionETC2) {
      textures.glass.loadFromFile(
          getAssetPath() + "textures/colored_glass_etc2_unorm.ktx",
  ...
  void setupVertexDescriptions() {
    ...
    vertices.attributeDescriptions = {
        ...
        // Location 3: UV
        vks::initializers::vertexInputAttributeDescription(
            VERTEX_BUFFER_BIND_ID, 3, VK_FORMAT_R32G32_SFLOAT,
            sizeof(float) * 9),
    };
  ...
  void setupDescriptorSetLayout() {
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayout, nullptr, &descriptorSetLayouts.scene));

    VkPipelineLayoutCreateInfo pPipelineLayoutCreateInfo =
        vks::initializers::pipelineLayoutCreateInfo(&descriptorSetLayouts.scene,
                                                    1);
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorSetAllocateInfo allocInfo =
        vks::initializers::descriptorSetAllocateInfo(
            descriptorPool, &descriptorSetLayouts.scene, 1);
  ...
  void preparePipelines() {
    ...
    VkGraphicsPipelineCreateInfo pipelineCreateInfo =
        vks::initializers::pipelineCreateInfo(pipelineLayouts.offscreen,
                                              renderPass, 0);
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/subpasses/gbuffer.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/subpasses/gbuffer.frag.spv",
  ...
  // Create the Vulkan objects used in the composition pass (descriptor sets,
  // pipelines, etc.)
  void prepareCompositionPass() {
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayout, nullptr, &descriptorSetLayouts.composition));

    // Pipeline layout
    VkPipelineLayoutCreateInfo pPipelineLayoutCreateInfo =
        vks::initializers::pipelineLayoutCreateInfo(
            &descriptorSetLayouts.composition, 1);
    ...
    VkDescriptorSetAllocateInfo allocInfo =
        vks::initializers::descriptorSetAllocateInfo(
            descriptorPool, &descriptorSetLayouts.composition, 1);
    ...
    VkDescriptorImageInfo texDescriptorPosition =
        vks::initializers::descriptorImageInfo(
            VK_NULL_HANDLE, attachments.position.view,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    VkDescriptorImageInfo texDescriptorNormal =
        vks::initializers::descriptorImageInfo(
            VK_NULL_HANDLE, attachments.normal.view,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    VkDescriptorImageInfo texDescriptorAlbedo =
        vks::initializers::descriptorImageInfo(
            VK_NULL_HANDLE, attachments.albedo.view,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        // Binding 0: Position texture target
        vks::initializers::writeDescriptorSet(
            descriptorSets.composition, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 0,
            &texDescriptorPosition),
        // Binding 1: Normals texture target
        vks::initializers::writeDescriptorSet(
            descriptorSets.composition, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 1,
            &texDescriptorNormal),
        // Binding 2: Albedo texture target
        vks::initializers::writeDescriptorSet(
            descriptorSets.composition, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 2,
            &texDescriptorAlbedo),
        // Binding 4: Fragment shader lights
        vks::initializers::writeDescriptorSet(
            descriptorSets.composition, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 3,
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/subpasses/composition.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/subpasses/composition.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    descriptorLayout = vks::initializers::descriptorSetLayoutCreateInfo(
        ...
        static_cast<uint32_t>(setLayoutBindings.size()));
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayout, nullptr, &descriptorSetLayouts.transparent));

    // Pipeline layout
    pPipelineLayoutCreateInfo = vks::initializers::pipelineLayoutCreateInfo(
        &descriptorSetLayouts.transparent, 1);
    VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pPipelineLayoutCreateInfo,
                                           nullptr,
                                           &pipelineLayouts.transparent));

    // Descriptor sets
    allocInfo = vks::initializers::descriptorSetAllocateInfo(
        descriptorPool, &descriptorSetLayouts.transparent, 1);
    VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo,
                                             &descriptorSets.transparent));

    writeDescriptorSets = {
        vks::initializers::writeDescriptorSet(
            descriptorSets.transparent, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
            &uniformBuffers.GBuffer.descriptor),
        vks::initializers::writeDescriptorSet(
            descriptorSets.transparent, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 1,
            &texDescriptorPosition),
        vks::initializers::writeDescriptorSet(
            descriptorSets.transparent,
            VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2,
            &textures.glass.descriptor),
    };
    vkUpdateDescriptorSets(device,
                           static_cast<uint32_t>(writeDescriptorSets.size()),
                           writeDescriptorSets.data(), 0, NULL);
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/subpasses/transparent.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/subpasses/transparent.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void initLights() {
    ...
    for (auto &light : uboLights.lights) {
      light.position = glm::vec4(rndDist(rndGen) * 6.0f,
                                 0.25f + std::abs(rndDist(rndGen)) * 4.0f,
                                 rndDist(rndGen) * 6.0f, 1.0f);
  ...
  // UI overlay configuration needs to be adjusted for this example (renderpass
  // setup, attachment count, etc.)
