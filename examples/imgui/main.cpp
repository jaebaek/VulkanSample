...
public:
  ...
  void initResources(VkRenderPass renderPass, VkQueue copyQueue) {
    ...
    shaderStages[0] = example->loadShader(
        ASSET_PATH "shaders/imgui/ui.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = example->loadShader(
        ASSET_PATH "shaders/imgui/ui.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...
public:
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      if (uiSettings.displayBackground) {
        ...
        vkCmdBindIndexBuffer(drawCmdBuffers[i],
                             models.background.indices.buffer, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(drawCmdBuffers[i], models.background.indexCount, 1, 0,
                         0, 0);
      }

      if (uiSettings.displayModels) {
        vkCmdBindVertexBuffers(drawCmdBuffers[i], 0, 1,
                               &models.models.vertices.buffer, offsets);
        vkCmdBindIndexBuffer(drawCmdBuffers[i], models.models.indices.buffer, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(drawCmdBuffers[i], models.models.indexCount, 1, 0, 0,
                         0);
      }

      if (uiSettings.displayLogos) {
        vkCmdBindVertexBuffers(drawCmdBuffers[i], 0, 1,
                               &models.logos.vertices.buffer, offsets);
        vkCmdBindIndexBuffer(drawCmdBuffers[i], models.logos.indices.buffer, 0,
                             VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(drawCmdBuffers[i], models.logos.indexCount, 1, 0, 0,
                         0);
      }
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(ASSET_PATH "shaders/imgui/scene.vert.spv",
                                 VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(ASSET_PATH "shaders/imgui/scene.frag.spv",
  ...
  void loadAssets() {
    models.models.loadFromFile(ASSET_PATH "models/vulkanscenemodels.dae",
                               vertexLayout, 1.0f, vulkanDevice, queue);
    models.background.loadFromFile(ASSET_PATH
                                   "models/vulkanscenebackground.dae",
                                   vertexLayout, 1.0f, vulkanDevice, queue);
    models.logos.loadFromFile(ASSET_PATH "models/vulkanscenelogos.dae",
                              vertexLayout, 1.0f, vulkanDevice, queue);
  }
