...
public:
  ...
  void prepareOffscreenRenderpass() {
    ...
    attachmentDescription.storeOp =
        VK_ATTACHMENT_STORE_OP_STORE; // We will read from depth, so it's
                                      // important to store the depth attachment
                                      // results
  ...
  // point-of-view to
  // The depth attachment of this framebuffer will then be used to sample from
  // in the fragment shader of the shadowing pass
  void prepareOffscreenFramebuffer() {
    ...
    image.usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
                  VK_IMAGE_USAGE_SAMPLED_BIT; // We will sample directly from
                                              // the depth attachment for the
                                              // shadow mapping
    VK_CHECK_RESULT(
...
#undef QUAD_COLOR_NORMAL
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/shadowmapping/offscreen.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/shadowmapping/offscreen.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void draw() {
    ...
    // (and transfer) to be finished before using the shadow map
