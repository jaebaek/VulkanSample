/*
* Vulkan Example - Example for VK_EXT_debug_marker extension. To be used in
* conjuction with a debugging app like RenderDoc (https://renderdoc.org)
*
...
void setup(VkDevice device, VkPhysicalDevice physicalDevice) {
  // Check if the debug marker extension is present (which is the case if run
  // from a graphics debugger)
  ...
  if (extensionPresent) {
    // The debug marker extension is not part of the core, so function pointers
    ...
    vkCmdDebugMarkerBegin = (PFN_vkCmdDebugMarkerBeginEXT)vkGetDeviceProcAddr(
    ...
    vkCmdDebugMarkerInsert = (PFN_vkCmdDebugMarkerInsertEXT)vkGetDeviceProcAddr(
        device, "vkCmdDebugMarkerInsertEXT");
    // Set flag if at least one function pointer is present
    active = (vkDebugMarkerSetObjectName != VK_NULL_HANDLE);
  } else {
    std::cout << "Warning: " << VK_EXT_DEBUG_MARKER_EXTENSION_NAME
              << " not present, debug markers are disabled.";
    std::cout
        << "Try running from inside a Vulkan graphics debugger (e.g. RenderDoc)"
        << std::endl;
  }
}
...
struct Scene {
  ...
  void draw(VkCommandBuffer cmdBuffer) {
    ...
    vkCmdBindIndexBuffer(cmdBuffer, model.indices.buffer, 0,
                         VK_INDEX_TYPE_UINT32);
    for (auto i = 0; i < model.parts.size(); i++) {
      // Add debug marker for mesh name
      DebugMarker::insert(cmdBuffer, "Draw \"" + modelPartNames[i] + "\"",
                          glm::vec4(0.0f));
      vkCmdDrawIndexed(cmdBuffer, model.parts[i].indexCount, 1,
                       model.parts[i].indexBase, 0, 0);
    }
  }

  void loadFromFile(std::string filename, vks::VulkanDevice *vulkanDevice,
                    VkQueue queue) {
...
public:
  ...
  void prepareOffscreen() {
    ...
    VK_CHECK_RESULT(vkBindImageMemory(device, offscreenPass.color.image,
                                      offscreenPass.color.mem, 0));
    ...
    DebugMarker::setObjectName(device, (uint64_t)offscreenPass.color.image,
                               VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
                               "Off-screen color framebuffer");
    DebugMarker::setObjectName(device, (uint64_t)offscreenPass.depth.image,
                               VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
                               "Off-screen depth framebuffer");
    DebugMarker::setObjectName(device, (uint64_t)offscreenPass.sampler,
                               VK_DEBUG_REPORT_OBJECT_TYPE_SAMPLER_EXT,
                               "Off-screen framebuffer default sampler");
  }
  ...
  void buildOffscreenCommandBuffer() {
    ...
    // Start a new debug marker region
    DebugMarker::beginRegion(offscreenPass.commandBuffer,
    ...
    vkCmdBindDescriptorSets(offscreenPass.commandBuffer,
                            VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0,
                            1, &descriptorSets.scene, 0, NULL);
    vkCmdBindPipeline(offscreenPass.commandBuffer,
                      VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.color);
  ...
  void loadScene() {
    scene.loadFromFile(getAssetPath() + "models/treasure_smooth.dae",
                       vulkanDevice, queue);
    sceneGlow.loadFromFile(getAssetPath() + "models/treasure_glow.dae",
                           vulkanDevice, queue);

    // Name the meshes
    // ASSIMP does not load mesh names from the COLLADA file used in this
    // example so we need to set them manually
    // These names are used in command buffer creation for setting debug markers
    std::vector<std::string> names = {"hill",
                                      "crystals",
                                      "rocks",
                                      "cave",
                                      "tree",
    ...
    // Scene
    DebugMarker::setObjectName(device, (uint64_t)scene.model.vertices.buffer,
                               VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
                               "Scene vertex buffer");
    DebugMarker::setObjectName(device, (uint64_t)scene.model.indices.buffer,
                               VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
                               "Scene index buffer");
    // Glow
    DebugMarker::setObjectName(
        device, (uint64_t)sceneGlow.model.vertices.buffer,
        VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, "Glow vertex buffer");
    DebugMarker::setObjectName(device, (uint64_t)sceneGlow.model.indices.buffer,
                               VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
                               "Glow index buffer");
  }
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      // Start a new debug marker region
      DebugMarker::beginRegion(drawCmdBuffers[i], "Render scene",
                               glm::vec4(0.5f, 0.76f, 0.34f, 1.0f));
      ...
      VkRect2D scissor = vks::initializers::rect2D(
          wireframe ? width / 2 : width, height, 0, 0);
      vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);

      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                              0, 1, &descriptorSets.scene, 0, NULL);

      // Solid rendering

      // Start a new debug marker region
      DebugMarker::beginRegion(drawCmdBuffers[i], "Toon shading draw",
                               glm::vec4(0.78f, 0.74f, 0.9f, 1.0f));

      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.toonshading);
      ...
      if (wireframe) {
        // Insert debug marker
        DebugMarker::beginRegion(drawCmdBuffers[i], "Wireframe draw",
                                 glm::vec4(0.53f, 0.78f, 0.91f, 1.0f));
      ...
      // Post processing
      if (glow) {
        DebugMarker::beginRegion(drawCmdBuffers[i], "Apply post processing",
                                 glm::vec4(0.93f, 0.89f, 0.69f, 1.0f));

        vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipelines.postprocess);
        // Full screen quad is generated by the vertex shaders, so we reuse four
        // vertices (for four invocations) from current vertex buffer
  ...
  void setupDescriptorSetLayout() {
    ...
    // Name for debugging
    DebugMarker::setObjectName(device, (uint64_t)pipelineLayout,
                               VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_LAYOUT_EXT,
                               "Shared pipeline layout");
    DebugMarker::setObjectName(
        device, (uint64_t)descriptorSetLayout,
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/debugmarker/toon.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/debugmarker/toon.frag.spv",
    ...
    // Color only pipeline
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/debugmarker/colorpass.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/debugmarker/colorpass.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/debugmarker/postprocess.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/debugmarker/postprocess.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 0],
                               VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                               "Toon shading vertex shader");
    DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 1],
                               VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                               "Toon shading fragment shader");
    DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 2],
                               VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                               "Color-only vertex shader");
    DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 3],
                               VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                               "Color-only fragment shader");
    DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 4],
                               VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                               "Postprocess vertex shader");
    DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 5],
                               VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                               "Postprocess fragment shader");

    // Name pipelines for debugging
    DebugMarker::setObjectName(device, (uint64_t)pipelines.toonshading,
                               VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
                               "Toon shading pipeline");
    DebugMarker::setObjectName(device, (uint64_t)pipelines.color,
                               VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
                               "Color only pipeline");
    DebugMarker::setObjectName(device, (uint64_t)pipelines.wireframe,
                               VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
                               "Wireframe rendering pipeline");
    DebugMarker::setObjectName(device, (uint64_t)pipelines.postprocess,
                               VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
  ...
  void prepareUniformBuffers() {
    ...
    DebugMarker::setObjectName(device, (uint64_t)uniformBuffer.buffer,
                               VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
                               "Scene uniform buffer block");
    // Add some random tag
    DebugMarker::setObjectTag(device, (uint64_t)uniformBuffer.buffer,
                              VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, 0,
                              sizeof(demoTag), &demoTag);
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Info")) {
      overlay->text("VK_EXT_debug_marker %s",
                    (DebugMarker::active ? "active" : "not present"));
    }
