...
public:
    ...
    for (uint32_t i = 0; i < numThreads; i++) {
      ...
      for (uint32_t j = 0; j < numObjectsPerThread; j++) {
        ...
        thread->objectData[j].pos =
            glm::vec3(sin(phi) * cos(theta), 0.0f, cos(phi)) * 35.0f;
        ...
        thread->objectData[j].rotationSpeed =
            (2.0f + rnd(4.0f)) * thread->objectData[j].rotationDir;
  ...
  void threadRenderCode(uint32_t threadIndex, uint32_t cmdBufferIndex,
    ...
    objectData->model =
        glm::rotate(objectData->model,
                    -sinf(glm::radians(objectData->deltaT * 360.0f)) * 0.25f,
                    glm::vec3(objectData->rotationDir, 0.0f, 0.0f));
    objectData->model =
        glm::rotate(objectData->model, glm::radians(objectData->rotation.y),
                    glm::vec3(0.0f, objectData->rotationDir, 0.0f));
    objectData->model = glm::rotate(
        objectData->model, glm::radians(objectData->deltaT * 360.0f),
        glm::vec3(0.0f, objectData->rotationDir, 0.0f));
    objectData->model =
        glm::scale(objectData->model, glm::vec3(objectData->scale));
    ...
    vkCmdBindVertexBuffers(cmdBuffer, 0, 1, &models.ufo.vertices.buffer,
                           offsets);
  ...
  void
  updateSecondaryCommandBuffer(VkCommandBufferInheritanceInfo inheritanceInfo) {
    ...
    vkCmdBindPipeline(secondaryCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
    ...
    VkDeviceSize offsets[1] = {0};
    vkCmdBindVertexBuffers(secondaryCommandBuffer, 0, 1,
                           &models.skysphere.vertices.buffer, offsets);
    vkCmdBindIndexBuffer(secondaryCommandBuffer,
                         models.skysphere.indices.buffer, 0,
                         VK_INDEX_TYPE_UINT32);
    vkCmdDrawIndexed(secondaryCommandBuffer, models.skysphere.indexCount, 1, 0,
                     0, 0);
  ...
  void loadMeshes() {
    models.ufo.loadFromFile(getAssetPath() + "models/retroufo_red_lowpoly.dae",
  ...
  void setupPipelineLayout() {
    ...
    VkPushConstantRange pushConstantRange =
        vks::initializers::pushConstantRange(
            VK_SHADER_STAGE_VERTEX_BIT, sizeof(ThreadPushConstantBlock), 0);
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/multithreading/phong.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/multithreading/phong.frag.spv",
