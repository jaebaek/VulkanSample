...
public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      // Second pass renders scaled object only where stencil was not set by
      // first pass
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.outline);
      vkCmdDrawIndexed(drawCmdBuffers[i], model.indexCount, 1, 0, 0, 0);
  ...
  void loadAssets() {
    model.loadFromFile(getAssetPath() + "models/venus.fbx", vertexLayout, 0.3f,
                       vulkanDevice, queue);
  }
  ...
  void preparePipelines() {
    ...
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            ...
            sizeof(float) * 6) // Location 2: Normal
    };
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/stencilbuffer/toon.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/stencilbuffer/toon.frag.spv",
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/stencilbuffer/outline.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/stencilbuffer/outline.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                              &pipelineCreateInfo, nullptr,
                                              &pipelines.outline));
