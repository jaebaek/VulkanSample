...
*
* Instead of using one uniform buffer per-object, this example allocates one big
* uniform buffer
* with respect to the alignment reported by the device via
* minUniformBufferOffsetAlignment that
* contains all matrices for the objects in the scene.
*
* The used descriptor type VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC then allows
* to set a dynamic
* offset used to pass data from the single uniform buffer to the connected
...
public:
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/dynamicuniformbuffer/base.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/dynamicuniformbuffer/base.frag.spv",
  ...
  void prepareUniformBuffers() {
    ...
    // We allocate this manually as the alignment of the offset differs between
    // GPUs
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        &uniformBuffers.dynamic, bufferSize));
    ...
    for (uint32_t i = 0; i < OBJECT_INSTANCES; i++) {
      rotations[i] = glm::vec3(rndDist(rndEngine), rndDist(rndEngine),
                               rndDist(rndEngine)) *
                     2.0f * (float)M_PI;
      rotationSpeeds[i] =
  ...
  void updateDynamicUniformBuffer(bool force = false) {
    ...
    for (uint32_t x = 0; x < dim; x++) {
      for (uint32_t y = 0; y < dim; y++) {
        for (uint32_t z = 0; z < dim; z++) {
          ...
          // Aligned offset
          glm::mat4 *modelMat = (glm::mat4 *)(((uint64_t)uboDataDynamic.model +
          ...
          *modelMat = glm::rotate(*modelMat, rotations[index].x,
                                  glm::vec3(1.0f, 1.0f, 0.0f));
          *modelMat = glm::rotate(*modelMat, rotations[index].y,
                                  glm::vec3(0.0f, 1.0f, 0.0f));
          *modelMat = glm::rotate(*modelMat, rotations[index].z,
    ...
    memcpy(uniformBuffers.dynamic.mapped, uboDataDynamic.model,
           uniformBuffers.dynamic.size);
