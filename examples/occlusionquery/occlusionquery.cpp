...
public:
  ...
  void getQueryResults() {
    ...
    vkGetQueryPoolResults(
        device, queryPool, 0, 2, sizeof(passedSamples), passedSamples,
        sizeof(uint64_t),
        // Store results a 64 bit values and wait until the results have been
        // finished
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                              0, 1, &descriptorSets.teapot, 0, NULL);
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
      ...
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                              0, 1, &descriptorSets.sphere, 0, NULL);
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.sphere.vertices.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.sphere.indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
      ...
      vkCmdClearAttachments(drawCmdBuffers[i], 2, clearAttachments, 1,
                            &clearRect);
  ...
  void loadAssets() {
    models.plane.loadFromFile(getAssetPath() + "models/plane_z.3ds",
                              vertexLayout, 0.4f, vulkanDevice, queue);
    models.teapot.loadFromFile(getAssetPath() + "models/teapot.3ds",
                               vertexLayout, 0.3f, vulkanDevice, queue);
    models.sphere.loadFromFile(getAssetPath() + "models/sphere.3ds",
                               vertexLayout, 0.3f, vulkanDevice, queue);
  }
  ...
  void setupDescriptorSets() {
    ...
    // Teapot
    VK_CHECK_RESULT(
        vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.teapot));
    ...
    // Sphere
    VK_CHECK_RESULT(
        vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.sphere));
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/occlusionquery/mesh.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/occlusionquery/mesh.frag.spv",
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/occlusionquery/simple.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/occlusionquery/simple.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/occlusionquery/occluder.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/occlusionquery/occluder.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   ...
                                   &uniformBuffers.occluder, sizeof(uboVS)));
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.teapot, sizeof(uboVS)));
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.sphere, sizeof(uboVS)));
  ...
  void updateUniformBuffers() {
    ...
    rotMatrix = glm::rotate(rotMatrix, glm::radians(rotation.x),
                            glm::vec3(1.0f, 0.0f, 0.0f));
    rotMatrix = glm::rotate(rotMatrix, glm::radians(rotation.y),
                            glm::vec3(0.0f, 1.0f, 0.0f));
    rotMatrix = glm::rotate(rotMatrix, glm::radians(rotation.z),
