...
vks::VertexLayout vertexLayout = vks::VertexLayout({
    ...
    vks::VERTEX_COMPONENT_DUMMY_VEC4, vks::VERTEX_COMPONENT_DUMMY_VEC4,
});
...
public:
  ...
  // Store reference to the ASSIMP scene for accessing properties of it during
  // animation
  ...
  void loadBones(const aiMesh *pMesh, uint32_t vertexOffset,
    ...
    for (uint32_t i = 0; i < pMesh->mNumBones; i++) {
      ...
      for (uint32_t j = 0; j < pMesh->mBones[i]->mNumWeights; j++) {
        uint32_t vertexID =
            vertexOffset + pMesh->mBones[i]->mWeights[j].mVertexId;
  ...
  void update(float time) {
    ...
    float AnimationTime =
        fmod(TimeInTicks, (float)scene->mAnimations[0]->mDuration);
...
private:
  ...
  // Returns a 4x4 matrix with interpolated translation between current and next
  // frame
  aiMatrix4x4 interpolateTranslation(float time, const aiNodeAnim *pNodeAnim) {
    ...
    if (pNodeAnim->mNumPositionKeys == 1) {
      ...
      aiVectorKey nextFrame =
          pNodeAnim
              ->mPositionKeys[(frameIndex + 1) % pNodeAnim->mNumPositionKeys];

      float delta = (time - (float)currentFrame.mTime) /
                    (float)(nextFrame.mTime - currentFrame.mTime);
  ...
  // Returns a 4x4 matrix with interpolated rotation between current and next
  // frame
  aiMatrix4x4 interpolateRotation(float time, const aiNodeAnim *pNodeAnim) {
    ...
    if (pNodeAnim->mNumRotationKeys == 1) {
      ...
      aiQuatKey nextFrame =
          pNodeAnim
              ->mRotationKeys[(frameIndex + 1) % pNodeAnim->mNumRotationKeys];
  ...
  // Returns a 4x4 matrix with interpolated scaling between current and next
  // frame
  aiMatrix4x4 interpolateScale(float time, const aiNodeAnim *pNodeAnim) {
    ...
    if (pNodeAnim->mNumScalingKeys == 1) {
      ...
      aiVectorKey nextFrame =
          pNodeAnim
              ->mScalingKeys[(frameIndex + 1) % pNodeAnim->mNumScalingKeys];
  ...
  void readNodeHierarchy(float AnimationTime, const aiNode *pNode,
    ...
    if (pNodeAnim) {
      ...
      aiMatrix4x4 matTranslation =
          interpolateTranslation(AnimationTime, pNodeAnim);
...
public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindIndexBuffer(drawCmdBuffers[i],
                           skinnedMesh->vertexBuffer.indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
      vkCmdDrawIndexed(drawCmdBuffers[i], skinnedMesh->vertexBuffer.indexCount,
                       1, 0, 0, 0);

      // Floor
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                              0, 1, &descriptorSets.floor, 0, NULL);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.texture);
  ...
  void loadMesh() {
    ...
    skinnedMesh->scene =
        skinnedMesh->Importer.ReadFileFromMemory(meshData, size, 0);
    ...
    skinnedMesh->globalInverseTransform =
        skinnedMesh->scene->mRootNode->mTransformation;
    ...
    // Iterate through all meshes in the file and extract the vertex information
    // used in this demo
    vertexBase = 0;
    for (uint32_t m = 0; m < skinnedMesh->scene->mNumMeshes; m++) {
      for (uint32_t v = 0; v < skinnedMesh->scene->mMeshes[m]->mNumVertices;
           v++) {
        Vertex vertex;

        vertex.pos =
            glm::make_vec3(&skinnedMesh->scene->mMeshes[m]->mVertices[v].x);
        vertex.normal =
            glm::make_vec3(&skinnedMesh->scene->mMeshes[m]->mNormals[v].x);
        vertex.uv = glm::make_vec2(
            &skinnedMesh->scene->mMeshes[m]->mTextureCoords[0][v].x);
        vertex.color =
            (skinnedMesh->scene->mMeshes[m]->HasVertexColors(0))
                ? glm::make_vec3(
                      &skinnedMesh->scene->mMeshes[m]->mColors[0][v].r)
                : glm::vec3(1.0f);
      ...
      for (uint32_t f = 0; f < skinnedMesh->scene->mMeshes[m]->mNumFaces; f++) {
        for (uint32_t i = 0; i < 3; i++) {
          indexBuffer.push_back(
              skinnedMesh->scene->mMeshes[m]->mFaces[f].mIndices[i] +
              indexBase);
        }
      }
    }
    VkDeviceSize indexBufferSize = indexBuffer.size() * sizeof(uint32_t);
    skinnedMesh->vertexBuffer.indexCount =
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        &skinnedMesh->vertexBuffer.vertices, vertexBufferSize));
    // Index buffer
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &skinnedMesh->vertexBuffer.indices,
        indexBufferSize));
    ...
    vkCmdCopyBuffer(copyCmd, vertexStaging.buffer,
                    skinnedMesh->vertexBuffer.vertices.buffer, 1, &copyRegion);

    copyRegion.size = indexBufferSize;
    vkCmdCopyBuffer(copyCmd, indexStaging.buffer,
                    skinnedMesh->vertexBuffer.indices.buffer, 1, &copyRegion);
  ...
  void loadAssets() {
    models.floor.loadFromFile(getAssetPath() + "models/plane_z.obj",
                              vertexLayout, 512.0f, vulkanDevice, queue);
    ...
    textures.colorMap.loadFromFile(getAssetPath() + "textures/goblin" +
                                       texFormatSuffix + ".ktx",
                                   texFormat, vulkanDevice, queue);
    textures.floor.loadFromFile(getAssetPath() + "textures/trail" +
                                    texFormatSuffix + ".ktx",
                                texFormat, vulkanDevice, queue);
  }
  ...
  void preparePipelines() {
    ...
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            VERTEX_BUFFER_BIND_ID, 4, VK_FORMAT_R32G32B32A32_SFLOAT,
            sizeof(float) * 11), // Location 4: Bone weights
        vks::initializers::vertexInputAttributeDescription(
            VERTEX_BUFFER_BIND_ID, 5, VK_FORMAT_R32G32B32A32_SINT,
            sizeof(float) * 15), // Location 5: Bone IDs
    };
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/skeletalanimation/mesh.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/skeletalanimation/mesh.frag.spv",
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/skeletalanimation/texture.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/skeletalanimation/texture.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                              &pipelineCreateInfo, nullptr,
                                              &pipelines.texture));
  ...
  void prepareUniformBuffers() {
    ...
    vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                               &uniformBuffers.mesh, sizeof(uboVS));
    // Map persistant
    ...
    vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                   VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                               &uniformBuffers.floor, sizeof(uboFloor));
    // Map persistant
  ...
  void updateUniformBuffers(bool viewChanged) {
    if (viewChanged) {
      uboVS.projection = glm::perspective(
          glm::radians(60.0f), (float)width / (float)height, 0.1f, 1024.0f);

      glm::mat4 viewMatrix =
          glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, zoom));
      viewMatrix = glm::rotate(viewMatrix, glm::radians(90.0f),
      ...
      uboVS.view =
          ...
          glm::translate(glm::mat4(1.0f),
                         glm::vec3(cameraPos.x, -cameraPos.z, cameraPos.y) *
                             100.0f);
      uboVS.view = glm::rotate(uboVS.view, glm::radians(rotation.x),
                               glm::vec3(1.0f, 0.0f, 0.0f));
      uboVS.view = glm::rotate(uboVS.view, glm::radians(rotation.z),
                               glm::vec3(0.0f, 1.0f, 0.0f));
      uboVS.view = glm::rotate(uboVS.view, glm::radians(-rotation.y),
      ...
      uboFloor.model = glm::translate(
          glm::mat4(1.0f),
          glm::vec3(cameraPos.x, -cameraPos.z, cameraPos.y) * 100.0f);
      uboFloor.model = glm::rotate(uboFloor.model, glm::radians(rotation.x),
                                   glm::vec3(1.0f, 0.0f, 0.0f));
      uboFloor.model = glm::rotate(uboFloor.model, glm::radians(rotation.z),
                                   glm::vec3(0.0f, 1.0f, 0.0f));
      uboFloor.model = glm::rotate(uboFloor.model, glm::radians(-rotation.y),
                                   glm::vec3(0.0f, 0.0f, 1.0f));
      uboFloor.model =
          glm::translate(uboFloor.model, glm::vec3(0.0f, 0.0f, -1800.0f));
    ...
    for (uint32_t i = 0; i < skinnedMesh->boneTransforms.size(); i++) {
      uboVS.bones[i] =
          glm::transpose(glm::make_mat4(&skinnedMesh->boneTransforms[i].a1));
    }
  ...
  virtual void viewChanged() { updateUniformBuffers(true); }
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      overlay->sliderFloat("Animation speed", &skinnedMesh->animationSpeed,
                           0.0f, 10.0f);
    }
  }
};
