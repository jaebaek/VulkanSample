...
public:
  ...
  virtual void getEnabledFeatures() {
    ...
    if (deviceFeatures.pipelineStatisticsQuery) {
      ...
      vks::tools::exitFatal(
          "Selected GPU does not support pipeline statistics!",
  ...
  void setupQueryPool() {
    pipelineStatNames = {"Input assembly vertex count        ",
  ...
  void getQueryResults() {
    ...
    vkGetQueryPoolResults(device, queryPool, 0, 1, count * sizeof(uint64_t),
                          pipelineStats.data(), sizeof(uint64_t),
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdResetQueryPool(drawCmdBuffers[i], queryPool, 0,
                          static_cast<uint32_t>(pipelineStats.size()));
      ...
      for (int32_t y = 0; y < gridSize; y++) {
        for (int32_t x = 0; x < gridSize; x++) {
          glm::vec3 pos = glm::vec3(float(x - (gridSize / 2.0f)) * 2.5f, 0.0f,
                                    float(y - (gridSize / 2.0f)) * 2.5f);
  ...
  void preparePipelines() {
    ...
    VkPipelineRasterizationStateCreateInfo rasterizationState =
        vks::initializers::pipelineRasterizationStateCreateInfo(
            VK_POLYGON_MODE_FILL, cullMode, VK_FRONT_FACE_CLOCKWISE, 0);
    ...
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            ...
            sizeof(float) * 6) // Location 3 : Color
    };
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/pipelinestatistics/scene.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/pipelinestatistics/scene.frag.spv",
    ...
    if (tessellation) {
      ...
      shaderStages[2] = loadShader(
          getAssetPath() + "shaders/pipelinestatistics/scene.tesc.spv",
          VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
      shaderStages[3] = loadShader(
          getAssetPath() + "shaders/pipelinestatistics/scene.tese.spv",
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      ...
      std::vector<std::string> cullModeNames = {"None", "Front", "Back",
                                                "Back and front"};
