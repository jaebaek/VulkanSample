...
public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      VkViewport viewport = vks::initializers::viewport(
          splitScreen ? (float)width / 2.0f : (float)width, (float)height, 0.0f,
          1.0f);
      ...
      if (splitScreen) {
        ...
        vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                          wireframe ? pipelines.wirePassThrough
                                    : pipelines.solidPassThrough);
        vkCmdDrawIndexed(drawCmdBuffers[i], models.object.indexCount, 1, 0, 0,
                         0);
      ...
      vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        wireframe ? pipelines.wire : pipelines.solid);
  ...
  void loadAssets() {
    models.object.loadFromFile(getAssetPath() + "models/lowpoly/deer.dae",
                               vertexLayout, 1.0f, vulkanDevice, queue);
    if (deviceFeatures.textureCompressionBC) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "textures/deer_bc3_unorm.ktx",
          VK_FORMAT_BC3_UNORM_BLOCK, vulkanDevice, queue);
    } else if (deviceFeatures.textureCompressionASTC_LDR) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "textures/deer_astc_8x8_unorm.ktx",
          VK_FORMAT_ASTC_8x8_UNORM_BLOCK, vulkanDevice, queue);
    } else if (deviceFeatures.textureCompressionETC2) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "textures/deer_etc2_unorm.ktx",
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/tessellation/base.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/tessellation/base.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    shaderStages[2] =
        loadShader(getAssetPath() + "shaders/tessellation/pntriangles.tesc.spv",
                   VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
    shaderStages[3] =
        loadShader(getAssetPath() + "shaders/tessellation/pntriangles.tese.spv",
    ...
    shaderStages[2] =
        loadShader(getAssetPath() + "shaders/tessellation/passthrough.tesc.spv",
                   VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT);
    shaderStages[3] =
        loadShader(getAssetPath() + "shaders/tessellation/passthrough.tese.spv",
                   VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT);
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        &uniformBuffers.tessEval, sizeof(uboTessEval)));
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.tessControl, sizeof(uboTessControl)));
  ...
  void updateUniformBuffers() {
    ...
    uboTessEval.projection = glm::perspective(
        glm::radians(45.0f),
        (float)(width * ((splitScreen) ? 0.5f : 1.0f)) / (float)height, 0.1f,
        256.0f);
    viewMatrix = glm::translate(viewMatrix, glm::vec3(0.0f, 0.0f, zoom));

    uboTessEval.model = glm::mat4(1.0f);
    uboTessEval.model =
        viewMatrix * glm::translate(uboTessEval.model, cameraPos);
    uboTessEval.model = glm::rotate(uboTessEval.model, glm::radians(rotation.x),
                                    glm::vec3(1.0f, 0.0f, 0.0f));
    uboTessEval.model = glm::rotate(uboTessEval.model, glm::radians(rotation.y),
                                    glm::vec3(0.0f, 1.0f, 0.0f));
    uboTessEval.model = glm::rotate(uboTessEval.model, glm::radians(rotation.z),
    ...
    memcpy(uniformBuffers.tessControl.mapped, &uboTessControl,
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      if (overlay->inputFloat("Tessellation level", &uboTessControl.tessLevel,
                              0.25f, 2)) {
        updateUniformBuffers();
      }
      if (deviceFeatures.fillModeNonSolid) {
