...
// todo: Fallback for sampler selection on devices that don't support
...
public:
  ...
  // To demonstrate mip mapping and filtering this example uses separate
  // samplers
  std::vector<std::string> samplerNames{"No mip maps", "Mip maps (bilinear)",
                                        "Mip maps (anisotropic)"};
  ...
  void loadTexture(std::string fileName, VkFormat format,
                   bool forceLinearTiling) {
    ...
    texture.mipLevels =
        floor(log2(std::max(texture.width, texture.height))) + 1;
    ...
    // Optimal image will be used as destination for the copy, so we must
    // transfer from our initial undefined image layout to the transfer
    // destination layout
    vks::tools::setImageLayout(
        copyCmd, texture.image, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    VkCommandBuffer blitCmd = VulkanExampleBase::createCommandBuffer(
        VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
    ...
    for (int32_t i = 1; i < texture.mipLevels; i++) {
      ...
      vks::tools::setImageLayout(
          blitCmd, texture.image, VK_IMAGE_LAYOUT_UNDEFINED,
          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipSubRange,
          VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT);

      // Blit from previous level
      vkCmdBlitImage(blitCmd, texture.image,
                     VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, texture.image,
                     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageBlit,
                     VK_FILTER_LINEAR);

      // Transiton current mip level to transfer source for read in next
      // iteration
      vks::tools::setImageLayout(
          blitCmd, texture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    vks::tools::setImageLayout(blitCmd, texture.image,
                               VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                               texture.imageLayout, subresourceRange);
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.tunnel.vertices.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.tunnel.indices.buffer, 0,
  ...
  void loadAssets() {
    models.tunnel.loadFromFile(getAssetPath() + "models/tunnel_cylinder.dae",
                               vertexLayout, 1.0f, vulkanDevice, queue);
    loadTexture(getAssetPath() + "textures/metalplate_nomips_rgba.ktx",
  ...
  void setupDescriptorPool() {
    std::vector<VkDescriptorPoolSize> poolSizes = {
        vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                              1), // Vertex shader UBO
        vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
                                              1), // Sampled image
        vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_SAMPLER,
                                              3), // 3 samplers (array)
    };
  ...
  void setupDescriptorSetLayout() {
    ...
    setLayoutBindings.push_back(vks::initializers::descriptorSetLayoutBinding(
        VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, VK_SHADER_STAGE_FRAGMENT_BIT, 1));

    // Binding 2: Sampler array (3 descriptors)
    setLayoutBindings.push_back(vks::initializers::descriptorSetLayoutBinding(
        VK_DESCRIPTOR_TYPE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 2, 3));
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorImageInfo textureDescriptor =
        vks::initializers::descriptorImageInfo(VK_NULL_HANDLE, texture.view,
                                               texture.imageLayout);
    writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(
        descriptorSet, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1,
        &textureDescriptor));

    // Binding 2: Sampler array
    std::vector<VkDescriptorImageInfo> samplerDescriptors;
    for (auto i = 0; i < samplers.size(); i++) {
      samplerDescriptors.push_back(vks::initializers::descriptorImageInfo(
    ...
    samplerDescriptorWrite.descriptorCount =
        static_cast<uint32_t>(samplerDescriptors.size());
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/texturemipmapgen/texture.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/texturemipmapgen/texture.frag.spv",
  ...
  void updateUniformBuffers() {
    ...
    uboVS.model = glm::rotate(glm::mat4(1.0f), glm::radians(timer * 360.0f),
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      ...
      if (overlay->comboBox("Sampler type", &uboVS.samplerIndex,
                            samplerNames)) {
        updateUniformBuffers();
      }
    }
