...
/*
        This example implements projective cascaded shadow mapping. This
   technique splits up the camera frustum into
        multiple frustums with each getting it's own full-res shadow map,
   implemented as a layered depth-only image.
        The shader then selects the proper shadow map layer depending on what
   split of the frustum the depth value
        to compare fits into.

        This results in a better shadow map resolution distribution that can be
   tweaked even further by increasing
        the number of frustum splits.

        A further optimization could be done using a geometry shader to do a
   single-pass render for the depth map
        cascades instead of multiple passes (geometry shaders are not supported
...
public:
  ...
  /*
          Render the example scene with given command buffer, pipeline layout
     and dscriptor set
          Used by the scene rendering and depth pass generation command buffer
  */
  void renderScene(VkCommandBuffer commandBuffer,
                   VkPipelineLayout pipelineLayout,
                   VkDescriptorSet descriptorSet, uint32_t cascadeIndex = 0) {
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            pipelineLayout, 0, 2, sets.data(), 0, NULL);
    vkCmdPushConstants(commandBuffer, pipelineLayout,
                       VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstBlock),
                       &pushConstBlock);
    vkCmdBindVertexBuffers(commandBuffer, 0, 1, &models[0].vertices.buffer,
                           offsets);
    vkCmdBindIndexBuffer(commandBuffer, models[0].indices.buffer, 0,
                         VK_INDEX_TYPE_UINT32);
    vkCmdDrawIndexed(commandBuffer, models[0].indexCount, 1, 0, 0, 0);

    // Trees
    const std::vector<glm::vec3> positions = {
        glm::vec3(0.0f, 0.0f, 0.0f),       glm::vec3(1.25f, 0.25f, 1.25f),
        glm::vec3(-1.25f, -0.2f, 1.25f),   glm::vec3(1.25f, 0.1f, -1.25f),
    ...
    for (auto position : positions) {
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                              pipelineLayout, 0, 2, sets.data(), 0, NULL);
      vkCmdBindVertexBuffers(commandBuffer, 0, 1, &models[1].vertices.buffer,
                             offsets);
      vkCmdBindIndexBuffer(commandBuffer, models[1].indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                              pipelineLayout, 0, 2, sets.data(), 0, NULL);
      vkCmdBindVertexBuffers(commandBuffer, 0, 1, &models[2].vertices.buffer,
                             offsets);
      vkCmdBindIndexBuffer(commandBuffer, models[2].indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
  ...
  /*
          ...
          The depth image is layered with each layer storing one shadow map
     cascade
  */
  void prepareDepthPass() {
    ...
    depthPass.commandBuffer = VulkanExampleBase::createCommandBuffer(
    ...
    */
    ...
    for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
      ...
      viewInfo.image = depth.image;
      VK_CHECK_RESULT(
          vkCreateImageView(device, &viewInfo, nullptr, &cascades[i].view));
      // Framebuffer
  ...
  /*
          ...
          Uses multiple passes with each pass rendering the scene to the
     cascade's depth image layer
          Could be optimized using a geometry shader (and layered frame buffer)
     on devices that support geometry shaders
  */
  void buildDepthPassCommandBuffer() {
    ...
    VkViewport viewport = vks::initializers::viewport(
        (float)SHADOWMAP_DIM, (float)SHADOWMAP_DIM, 0.0f, 1.0f);
    vkCmdSetViewport(depthPass.commandBuffer, 0, 1, &viewport);

    VkRect2D scissor =
        vks::initializers::rect2D(SHADOWMAP_DIM, SHADOWMAP_DIM, 0, 0);
    vkCmdSetScissor(depthPass.commandBuffer, 0, 1, &scissor);

    // One pass per cascade
    // The layer that this pass renders too is defined by the cascade's image
    // view (selected via the cascade's decsriptor set)
    for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
      renderPassBeginInfo.framebuffer = cascades[i].frameBuffer;
      vkCmdBeginRenderPass(depthPass.commandBuffer, &renderPassBeginInfo,
                           VK_SUBPASS_CONTENTS_INLINE);
      vkCmdBindPipeline(depthPass.commandBuffer,
                        VK_PIPELINE_BIND_POINT_GRAPHICS, depthPass.pipeline);
      renderScene(depthPass.commandBuffer, depthPass.pipelineLayout,
                  cascades[i].descriptorSet, i);
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); i++) {
      ...
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        (filterPCF) ? pipelines.sceneShadowPCF
  ...
  void loadAssets() {
    ...
    models.resize(3);
    models[0].loadFromFile(getAssetPath() + "models/terrain_simple.dae",
                           vertexLayout, 1.0f, vulkanDevice, queue);
    models[1].loadFromFile(getAssetPath() + "models/oak_trunk.dae",
                           vertexLayout, 2.0f, vulkanDevice, queue);
    models[2].loadFromFile(getAssetPath() + "models/oak_leafs.dae",
                           vertexLayout, 2.0f, vulkanDevice, queue);
  }
  ...
  void setupLayoutsAndDescriptors() {
    ...
    */
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayout, nullptr, &descriptorSetLayouts.base));
    ...
    descriptorLayout =
        vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayout, nullptr, &descriptorSetLayouts.material));
    ...
    */
    ...
    VkDescriptorImageInfo depthMapDescriptor =
        vks::initializers::descriptorImageInfo(
            depth.sampler, depth.view,
            VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);

    VkDescriptorSetAllocateInfo allocInfo =
        vks::initializers::descriptorSetAllocateInfo(
            descriptorPool, &descriptorSetLayouts.base, 1);
    ...
    writeDescriptorSets = {
        ...
        vks::initializers::writeDescriptorSet(descriptorSet,
                                              VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                              2, &uniformBuffers.FS.descriptor),
    };
    ...
    for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
      ...
      writeDescriptorSets = {
          vks::initializers::writeDescriptorSet(
              cascades[i].descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
              &depthPass.uniformBuffer.descriptor),
          vks::initializers::writeDescriptorSet(
              cascades[i].descriptorSet,
              VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &cascadeImageInfo)};
      vkUpdateDescriptorSets(device,
                             static_cast<uint32_t>(writeDescriptorSets.size()),
                             writeDescriptorSets.data(), 0, NULL);
    }
    ...
    */
    ...
    // Shared pipeline layout (scene and depth map debug display)
    {
      ...
      std::array<VkDescriptorSetLayout, 2> setLayouts = {
          descriptorSetLayouts.base, descriptorSetLayouts.material};
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/shadowmappingcascade/debugshadowmap.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/shadowmappingcascade/debugshadowmap.frag.spv",
    ...
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
        ...
        vks::initializers::vertexInputAttributeDescription(
            0, 1, VK_FORMAT_R32G32_SFLOAT, sizeof(float) * 3), // Location 1: UV
        vks::initializers::vertexInputAttributeDescription(
    ...
    */
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/shadowmappingcascade/scene.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/shadowmappingcascade/scene.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    */
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/shadowmappingcascade/depthpass.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/shadowmappingcascade/depthpass.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        &depthPass.uniformBuffer, sizeof(depthPass.ubo)));
  ...
  /*
          Calculate frustum split depths and matrices for the shadow map
     cascades
          Based on
  ...
  */
  void updateCascades() {
    ...
    for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
      ...
      glm::mat4 invCam =
          glm::inverse(camera.matrices.perspective * camera.matrices.view);
      ...
      cascades[i].splitDepth =
          (camera.getNearClip() + splitDist * clipRange) * -1.0f;
  ...
  void updateUniformBuffers() {
    ...
    */
    ...
    memcpy(depthPass.uniformBuffer.mapped, &depthPass.ubo,
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      if (overlay->sliderFloat("Split lambda", &cascadeSplitLambda, 0.1f,
                               1.0f)) {
        updateCascades();
        updateUniformBuffers();
      }
      ...
      if (displayDepthMap) {
        if (overlay->sliderInt("Cascade", &displayDepthMapCascadeIndex, 0,
                               SHADOW_MAP_CASCADE_COUNT - 1)) {
          buildCommandBuffers();
        }
      }
