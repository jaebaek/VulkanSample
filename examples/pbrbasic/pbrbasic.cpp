...
*
* See http://graphicrants.blogspot.de/2013/08/specular-brdf-reference.html for a
* good reference to the different functions that make up a specular BRDF
*
...
public:
  ...
  VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION) {
    ...
    materials.push_back(Material(
        "Copper", glm::vec3(0.955008f, 0.637427f, 0.538163f), 0.1f, 1.0f));
    materials.push_back(Material(
        "Chromium", glm::vec3(0.549585f, 0.556114f, 0.554256f), 0.1f, 1.0f));
    materials.push_back(Material(
        "Nickel", glm::vec3(0.659777f, 0.608679f, 0.525649f), 0.1f, 1.0f));
    materials.push_back(Material(
        "Titanium", glm::vec3(0.541931f, 0.496791f, 0.449419f), 0.1f, 1.0f));
    materials.push_back(Material(
        "Cobalt", glm::vec3(0.662124f, 0.654864f, 0.633732f), 0.1f, 1.0f));
    materials.push_back(Material(
        "Platinum", glm::vec3(0.672411f, 0.637331f, 0.585456f), 0.1f, 1.0f));
    // Testing materials
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindVertexBuffers(
          drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
...
//#define SINGLE_ROW 1
      ...
      for (uint32_t x = 0; x < objcount; x++) {
        glm::vec3 pos =
            glm::vec3(float(x - (objcount / 2.0f)) * 2.5f, 0.0f, 0.0f);
      ...
      for (uint32_t y = 0; y < GRID_DIM; y++) {
        for (uint32_t x = 0; x < GRID_DIM; x++) {
          ...
          vkCmdPushConstants(drawCmdBuffers[i], pipelineLayout,
                             ...
                             &pos);
          mat.params.metallic =
              glm::clamp((float)x / (float)(GRID_DIM - 1), 0.1f, 1.0f);
          mat.params.roughness =
              glm::clamp((float)y / (float)(GRID_DIM - 1), 0.05f, 1.0f);
          vkCmdPushConstants(drawCmdBuffers[i], pipelineLayout,
                             VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(glm::vec3),
                             sizeof(Material::PushBlock), &mat);
          vkCmdDrawIndexed(drawCmdBuffers[i],
  ...
  void loadAssets() {
    ...
    for (auto file : filenames) {
      ...
      model.loadFromFile(getAssetPath() + "models/" + file, vertexLayout,
                         OBJ_DIM * (file == "venus.fbx" ? 3.0f : 1.0f),
                         vulkanDevice, queue);
  ...
  void preparePipelines() {
    ...
    // PBR pipeline
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/pbrbasic/pbr.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/pbrbasic/pbr.frag.spv",
