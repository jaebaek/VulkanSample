...
public:
  ...
  // Create an image memory barrier used to change the layout of an image and
  // put it into an active command buffer
  void setImageLayout(VkCommandBuffer cmdBuffer, VkImage image,
    ...
    // For a more complete version that can be used with other layouts see
    ...
    vkCmdPipelineBarrier(cmdBuffer, srcStageFlags, destStageFlags,
                         VK_FLAGS_NONE, 0, nullptr, 0, nullptr, 1,
                         &imageMemoryBarrier);
  }
  ...
  void loadTexture() {
    ...
    // This is just for learning purposes and not suggested, as linear tiled
    // images are pretty restricted and often only support a small set of
    // features (e.g. no mips, etc.)
    ...
    if (useStaging) {
      ...
      setImageLayout(copyCmd, texture.image, VK_IMAGE_ASPECT_COLOR_BIT,
                     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, texture.imageLayout,
                     subresourceRange);
      ...
      // Start at first mip level
      subresourceRange.baseMipLevel = 0;
      // Only one mip level, most implementations won't support more for linear
      // tiled images
  ...
  void setupDescriptorSet() {
    ...
    textureDescriptor.imageView =
        texture.view; // The image's view (images are never directly accessed by
                      // the shader, but rather through views defining
                      // subresources)
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/texture/texture.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/texture/texture.frag.spv",
