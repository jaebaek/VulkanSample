...
public:
  ...
  struct {
    VkDescriptorSetLayout
        descriptorSetLayout; // Raytraced image display shader binding layout
    VkDescriptorSet descriptorSetPreCompute; // Raytraced image display shader
                                             // bindings before compute shader
                                             // image manipulation
    VkDescriptorSet descriptorSet;   // Raytraced image display shader bindings
  ...
  struct Sphere { // Shader uses std140 layout (so we only use vec4 instead of
                  // vec3)
    glm::vec3 pos;
    float radius;
    glm::vec3 diffuse;
  ...
  void prepareTextureTarget(vks::Texture *tex, uint32_t width, uint32_t height,
                            VkFormat format) {
  ...
  Sphere newSphere(glm::vec3 pos, float radius, glm::vec3 diffuse,
                   float specular) {
    Sphere sphere;
    sphere.id = currentId++;
    sphere.pos = pos;
  ...
  Plane newPlane(glm::vec3 normal, float distance, glm::vec3 diffuse,
                 float specular) {
    Plane plane;
    plane.id = currentId++;
    plane.normal = normal;
  ...
  void prepareStorageBuffers() {
    ...
    spheres.push_back(newSphere(glm::vec3(1.75f, -0.5f, 0.0f), 1.0f,
  ...
  void setupDescriptorPool() {
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        vks::initializers::descriptorPoolSize(
            ...
            4), // Graphics image samplers
        vks::initializers::descriptorPoolSize(
            VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
            1), // Storage image for ray traced image output
        vks::initializers::descriptorPoolSize(
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            2), // Storage buffer for the scene primitives
    };
