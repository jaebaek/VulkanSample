...
*
* Note: Requires a device that supports the VK_EXT_conservative_rasterization
* extension
*
* Uses an offscreen buffer with lower resolution to demonstrate the effect of
...
public:
  ...
  VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION) {
    ...
    // Reading device properties of conservative rasterization requires
    // VK_KHR_get_physical_device_properties2 to be enabled
  ...
  ~VulkanExample() {
    ...
    vkDestroyDescriptorSetLayout(device, descriptorSetLayouts.fullscreen,
                                 nullptr);
  ...
  /*
          Setup offscreen framebuffer, attachments and render passes for lower
     resolution rendering of the scene
  */
  ...
  void buildOffscreenCommandBuffer() {
    ...
    vkCmdBindDescriptorSets(
        ...
        pipelineLayouts.scene, 0, 1, &descriptorSets.scene, 0, nullptr);
    vkCmdBindPipeline(
        offscreenPass.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
        conservativeRasterEnabled ? pipelines.triangleConservativeRaster
                                  : pipelines.triangle);

    VkDeviceSize offsets[1] = {0};
    vkCmdBindVertexBuffers(offscreenPass.commandBuffer, 0, 1,
                           &triangle.vertices.buffer, offsets);
    vkCmdBindIndexBuffer(offscreenPass.commandBuffer, triangle.indices.buffer,
                         0, VK_INDEX_TYPE_UINT32);
    vkCmdSetViewport(offscreenPass.commandBuffer, 0, 1, &viewport);
    vkCmdDrawIndexed(offscreenPass.commandBuffer, triangle.indexCount, 1, 0, 0,
                     0);
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              ...
                              pipelineLayouts.fullscreen, 0, 1,
                              &descriptorSets.fullscreen, 0, nullptr);
      vkCmdDraw(drawCmdBuffers[i], 3, 1, 0, 0);

      // Overlay actual triangle
      VkDeviceSize offsets[1] = {0};
      vkCmdBindVertexBuffers(drawCmdBuffers[i], 0, 1, &triangle.vertices.buffer,
                             offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], triangle.indices.buffer, 0,
  ...
  void loadAssets() {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &triangle.vertices,
        vertexBufferSize));
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &triangle.indices,
        indexBufferSize));

    // Copy from host do device
    vulkanDevice->copyBuffer(&stagingBuffers.vertices, &triangle.vertices,
                             queue);
  ...
  void setupDescriptorSetLayout() {
    ...
    descriptorLayout = vks::initializers::descriptorSetLayoutCreateInfo(
        ...
        static_cast<uint32_t>(setLayoutBindings.size()));
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayout, nullptr, &descriptorSetLayouts.fullscreen));
    pPipelineLayoutCreateInfo = vks::initializers::pipelineLayoutCreateInfo(
        &descriptorSetLayouts.fullscreen, 1);
    VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pPipelineLayoutCreateInfo,
                                           nullptr,
  ...
  void setupDescriptorSet() {
    ...
    // Fullscreen pass
    descriptorSetAllocInfo = vks::initializers::descriptorSetAllocateInfo(
        descriptorPool, &descriptorSetLayouts.fullscreen, 1);
    VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &descriptorSetAllocInfo,
                                             &descriptorSets.fullscreen));
  ...
  void preparePipelines() {
    ...
    VkGraphicsPipelineCreateInfo pipelineCreateInfo =
        vks::initializers::pipelineCreateInfo(pipelineLayouts.fullscreen,
                                              renderPass, 0);
    ...
    */
    ...
    /*
            ...
            Requires VK_KHR_get_physical_device_properties2 and manual function
       pointer creation
    */
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/conservativeraster/fullscreen.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/conservativeraster/fullscreen.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    // Empty vertex input state (full screen triangle generated in vertex
    // shader)
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/conservativeraster/triangle.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/conservativeraster/triangleoverlay.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                              &pipelineCreateInfo, nullptr,
                                              &pipelines.triangleOverlay));
    ...
    */
    ...
    shaderStages[0] = loadShader(
        ...
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/conservativeraster/triangle.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    */
    ...
    // Conservative rasterization state has to be chained into the pipeline
    // rasterization state create info structure
    rasterizationStateCI.pNext = &conservativeRasterStateCI;

    VK_CHECK_RESULT(vkCreateGraphicsPipelines(
        device, pipelineCache, 1, &pipelineCreateInfo, nullptr,
        &pipelines.triangleConservativeRaster));
  }
  ...
  void prepareUniformBuffers() {
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.scene, sizeof(uboScene)));
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      if (overlay->checkBox("Conservative rasterization",
    ...
    if (overlay->header("Device properties")) {
      overlay->text(
          ...
          conservativeRasterProps.maxExtraPrimitiveOverestimationSize);
      overlay->text(
          "extraPrimitiveOverestimationSizeGranularity: %f",
          conservativeRasterProps.extraPrimitiveOverestimationSizeGranularity);
      overlay->text("primitiveUnderestimation:                    %s",
                    conservativeRasterProps.primitiveUnderestimation ? "yes"
                                                                     : "no");
      overlay->text(
          "conservativePointAndLineRasterization:       %s",
          conservativeRasterProps.conservativePointAndLineRasterization ? "yes"
                                                                        : "no");
      overlay->text(
          "degenerateTrianglesRasterized:               %s",
          conservativeRasterProps.degenerateTrianglesRasterized ? "yes" : "no");
      overlay->text("degenerateLinesRasterized:                   %s",
                    conservativeRasterProps.degenerateLinesRasterized ? "yes"
                                                                      : "no");
      overlay->text(
          "fullyCoveredFragmentShaderInputVariable:     %s",
          conservativeRasterProps.fullyCoveredFragmentShaderInputVariable
              ? "yes"
              : "no");
      overlay->text(
          "conservativeRasterizationPostDepthCoverage:  %s",
          conservativeRasterProps.conservativeRasterizationPostDepthCoverage
              ? "yes"
              : "no");
    }
  }
