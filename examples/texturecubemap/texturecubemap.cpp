...
public:
  ...
  void loadCubemap(std::string filename, VkFormat format,
    ...
    VK_CHECK_RESULT(
        vkCreateImage(device, &imageCreateInfo, nullptr, &cubeMap.image));
    ...
    vks::tools::setImageLayout(
        copyCmd, cubeMap.image, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);

    // Copy the cube map faces from the staging buffer to the optimal tiled
    // image
    vkCmdCopyBufferToImage(copyCmd, stagingBuffer, cubeMap.image,
    ...
    vks::tools::setImageLayout(copyCmd, cubeMap.image,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                               cubeMap.imageLayout, subresourceRange);
  ...
  void loadTextures() {
    ...
    // As the support differs between implemementations we need to check device
    // features and select a proper format and file
    std::string filename;
    VkFormat format;
    if (deviceFeatures.textureCompressionBC) {
  ...
  void loadAssets() {
    ...
    // Objects
    std::vector<std::string> filenames = {"sphere.obj", "teapot.dae",
  ...
  void setupDescriptorSets() {
    ...
    VkDescriptorImageInfo textureDescriptor =
        vks::initializers::descriptorImageInfo(cubeMap.sampler, cubeMap.view,
                                               cubeMap.imageLayout);
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/texturecubemap/skybox.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/texturecubemap/skybox.frag.spv",
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/texturecubemap/reflect.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/texturecubemap/reflect.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.object, sizeof(uboVS)));
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.skybox, sizeof(uboVS)));
  ...
  void updateUniformBuffers() {
    ...
    uboVS.projection = glm::perspective(
        glm::radians(60.0f), (float)width / (float)height, 0.001f, 256.0f);
