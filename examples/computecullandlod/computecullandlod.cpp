...
public:
  ...
  struct {
    ...
    uint32_t lodCount[MAX_LOD_LEVEL + 1]; // Statistics for number of draws per
                                          // LOD level (written by compute
                                          // shader)
  } indirectStats;
  ...
  struct {
    vks::Buffer lodLevelsBuffers; // Contains index start and counts for the
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.lodObject.vertices.buffer, offsets);
      vkCmdBindVertexBuffers(drawCmdBuffers[i], INSTANCE_BUFFER_BIND_ID, 1,
                             &instanceBuffer.buffer, offsets);

      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.lodObject.indices.buffer,
                           0, VK_INDEX_TYPE_UINT32);

      if (vulkanDevice->features.multiDrawIndirect) {
        vkCmdDrawIndexedIndirect(
            drawCmdBuffers[i], indirectCommandsBuffer.buffer, 0,
            indirectStats.drawCount, sizeof(VkDrawIndexedIndirectCommand));
      } else {
  ...
  void loadAssets() {
    models.lodObject.loadFromFile(getAssetPath() + "models/suzanne_lods.dae",
  ...
  void setupVertexDescriptions() {
    ...
    vertices.attributeDescriptions.push_back(
        vks::initializers::vertexInputAttributeDescription(
            INSTANCE_BUFFER_BIND_ID, 5, VK_FORMAT_R32_SFLOAT,
  ...
  void buildComputeCommandBuffer() {
    ...
    // Add memory barrier to ensure that the indirect commands have been
    // consumed before the compute shader updates them
    ...
    // The compute shader will do the frustum culling and adjust the indirect
    // draw calls depending on object visibility.
    ...
    // Add memory barrier to ensure that the compute shader has finished writing
    // the indirect command buffer before it's consumed
    ...
    vkCmdPipelineBarrier(compute.commandBuffer,
                         ...
                         VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT, VK_FLAGS_NONE, 0,
                         nullptr, 1, &bufferBarrier, 0, nullptr);
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/computecullandlod/indirectdraw.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/computecullandlod/indirectdraw.frag.spv",
  ...
  void prepareBuffers() {
    ...
    for (uint32_t x = 0; x < OBJECT_COUNT; x++) {
      for (uint32_t y = 0; y < OBJECT_COUNT; y++) {
        for (uint32_t z = 0; z < OBJECT_COUNT; z++) {
          uint32_t index =
              x + y * OBJECT_COUNT + z * OBJECT_COUNT * OBJECT_COUNT;
    ...
    for (uint32_t x = 0; x < OBJECT_COUNT; x++) {
      for (uint32_t y = 0; y < OBJECT_COUNT; y++) {
        for (uint32_t z = 0; z < OBJECT_COUNT; z++) {
          uint32_t index =
              x + y * OBJECT_COUNT + z * OBJECT_COUNT * OBJECT_COUNT;
          instanceData[index].pos = glm::vec3((float)x, (float)y, (float)z) -
                                    glm::vec3((float)OBJECT_COUNT / 2.0f);
    ...
    for (auto modelPart : models.lodObject.parts) {
      ...
      lod.distance =
          5.0f + n * 5.0f; // Starting distance (to viewer) for this LOD
      n++;
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &stagingBuffer, LODLevels.size() * sizeof(LOD), LODLevels.data()));

    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &compute.lodLevelsBuffers,
        stagingBuffer.size));
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformData.scene, sizeof(uboScene)));
  ...
  void prepareCompute() {
    ...
    std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
        ...
        // Binding 4: LOD info
        vks::initializers::writeDescriptorSet(
            compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 4,
    ...
    computePipelineCreateInfo.stage =
        loadShader(getAssetPath() + "shaders/computecullandlod/cull.comp.spv",
                   VK_SHADER_STAGE_COMPUTE_BIT);

    // Use specialization constants to pass max. level of detail (determined by
    // no. of meshes)
    ...
    uint32_t specializationData =
        static_cast<uint32_t>(models.lodObject.parts.size()) - 1;
  ...
  void updateUniformBuffer(bool viewChanged) {
    if (viewChanged) {
      ...
      if (!fixedFrustum) {
        ...
        memcpy(uboScene.frustumPlanes, frustum.planes.data(),
               sizeof(glm::vec4) * 6);
      }
    }
  ...
  void draw() {
    ...
    memcpy(&indirectStats, indirectDrawCountBuffer.mapped,
