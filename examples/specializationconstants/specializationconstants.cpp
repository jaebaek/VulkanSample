...
public:
  ...
  VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION) {
    ...
    camera.setPerspective(60.0f, ((float)width / 3.0f) / (float)height, 0.1f,
                          512.0f);
  ...
  void loadAssets() {
    models.cube.loadFromFile(getAssetPath() + "models/color_teapot_spheres.dae",
  ...
  void preparePipelines() {
    ...
    // Shader bindings based on specialization constants are marked by the new
    ...
    specializationMapEntries[1].size =
        sizeof(specializationData.toonDesaturationFactor);
    ...
    // Create pipelines
    // All pipelines will use the same "uber" shader and specialization
    // constants to change branching and parameters of that shader
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/specializationconstants/uber.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/specializationconstants/uber.frag.spv",
        VK_SHADER_STAGE_FRAGMENT_BIT);
    // Specialization info is assigned is part of the shader stage (modul) and
    // must be set after creating the module and before creating the pipeline
