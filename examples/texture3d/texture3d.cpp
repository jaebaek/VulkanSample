/*
* Vulkan Example - 3D texture loading (and generation using perlin noise)
* example
*
...
template <typename T> class PerlinNoise {
private:
...
public:
  PerlinNoise() {
    // Generate random lookup for permutations containing all numbers from
    // 0..255
  ...
  T noise(T x, T y, T z) {
    ...
    T res = lerp(w, lerp(v, lerp(u, grad(permutations[AA], x, y, z),
...
template <typename T> class FractalNoise {
private:
...
public:
  ...
  T noise(T x, T y, T z) {
    ...
    for (int32_t i = 0; i < octaves; i++) {
      sum += perlinNoise.noise(x * frequency, y * frequency, z * frequency) *
             amplitude;
      max += amplitude;
...
public:
  ...
  void prepareNoiseTexture(uint32_t width, uint32_t height, uint32_t depth) {
    ...
    uint32_t maxImageDimension3D(
        vulkanDevice->properties.limits.maxImageDimension3D);
  ...
  void updateNoiseTexture() {
    ...
    std::cout << "Generating " << texture.width << " x " << texture.height
              << " x " << texture.depth << " noise texture..." << std::endl;
...
#define FRACTAL
#ifdef FRACTAL
          float n = fractalNoise.noise(nx * noiseScale, ny * noiseScale,
                                       nz * noiseScale);
#else
          ...
          data[x + y * texture.width + z * texture.width * texture.height] =
  ...
  void setupVertexDescriptions() {
    ...
    vertices.inputBinding[0] = vks::initializers::vertexInputBindingDescription(
    ...
    vertices.inputState.vertexBindingDescriptionCount =
        static_cast<uint32_t>(vertices.inputBinding.size());
    vertices.inputState.pVertexBindingDescriptions =
        vertices.inputBinding.data();
    vertices.inputState.vertexAttributeDescriptionCount =
        static_cast<uint32_t>(vertices.inputAttributes.size());
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/texture3d/texture3d.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/texture3d/texture3d.frag.spv",
