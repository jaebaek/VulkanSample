/*
* Vulkan Example - Push constants example (small shader block accessed outside
* of uniforms for fast updates)
*
...
#define y -4.0f
      ...
      pushConstants[5] =
          glm::vec4(r * 2.5f * cos_t, y, r * 2.5f * sin_t, 1.25f);
#undef r
#undef y
#undef sin_t
#undef cos_t
  ...
  void loadAssets() {
    models.scene.loadFromFile(getAssetPath() + "models/samplescene.dae",
                              vertexLayout, 0.35f, vulkanDevice, queue);
  }
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/pushconstants/lights.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/pushconstants/lights.frag.spv",
  ...
  void prepare() {
    ...
    // Specs require 128 bytes, so if the device complies our push constant
    // buffer should always fit into memory
    assert(sizeof(pushConstants) <=
           vulkanDevice->properties.limits.maxPushConstantsSize);
