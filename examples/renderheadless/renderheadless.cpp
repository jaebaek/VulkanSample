...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
#define LOG(...)                                                               \
  ((void)__android_log_print(ANDROID_LOG_INFO, "vulkanExample", __VA_ARGS__))
#else
    ...
    */
    {
      ...
      attchmentDescriptions[0].stencilStoreOp =
      ...
      attchmentDescriptions[1].stencilStoreOp =
          VK_ATTACHMENT_STORE_OP_DONT_CARE;
...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
      shaderStages[0].module = vks::tools::loadShader(
          androidapp->activity->assetManager,
          ASSET_PATH "shaders/renderheadless/triangle.vert.spv", device);
      shaderStages[1].module = vks::tools::loadShader(
          androidapp->activity->assetManager,
          ASSET_PATH "shaders/renderheadless/triangle.frag.spv", device);
#else
      shaderStages[0].module = vks::tools::loadShader(
          ASSET_PATH "shaders/renderheadless/triangle.vert.spv", device);
      shaderStages[1].module = vks::tools::loadShader(
    ...
    */
    {
      ...
      std::vector<glm::vec3> pos = {
          glm::vec3(-1.5f, 0.0f, -4.0f), glm::vec3(0.0f, 0.0f, -2.5f),
...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
      const char *filename =
          strcat(getenv("EXTERNAL_STORAGE"), "/headless.ppm");
#else
