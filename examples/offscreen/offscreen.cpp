...
public:
  ...
  ~VulkanExample() {
    ...
    vkDestroyDescriptorSetLayout(device, descriptorSetLayouts.textured,
                                 nullptr);
  ...
  void buildOffscreenCommandBuffer() {
    ...
    vkCmdBindDescriptorSets(
        ...
        pipelineLayouts.shaded, 0, 1, &descriptorSets.offscreen, 0, NULL);
    ...
    vkCmdBindVertexBuffers(offscreenPass.commandBuffer, VERTEX_BUFFER_BIND_ID,
                           1, &models.example.vertices.buffer, offsets);
    vkCmdBindIndexBuffer(offscreenPass.commandBuffer,
                         models.example.indices.buffer, 0,
                         VK_INDEX_TYPE_UINT32);
    vkCmdDrawIndexed(offscreenPass.commandBuffer, models.example.indexCount, 1,
                     0, 0, 0);
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      if (debugDisplay) {
        vkCmdBindDescriptorSets(
            ...
            pipelineLayouts.textured, 0, 1, &descriptorSets.debugQuad, 0, NULL);
      ...
      vkCmdBindDescriptorSets(
          drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
          pipelineLayouts.textured, 0, 1, &descriptorSets.mirror, 0, NULL);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.mirror);

      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.plane.vertices.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.plane.indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
      ...
      vkCmdBindDescriptorSets(
          drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
          pipelineLayouts.shaded, 0, 1, &descriptorSets.model, 0, NULL);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.shaded);

      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.example.vertices.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.example.indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
      vkCmdDrawIndexed(drawCmdBuffers[i], models.example.indexCount, 1, 0, 0,
                       0);
  ...
  void loadAssets() {
    models.plane.loadFromFile(getAssetPath() + "models/plane.obj", vertexLayout,
                              0.5f, vulkanDevice, queue);
    models.example.loadFromFile(getAssetPath() + "models/chinesedragon.dae",
                                vertexLayout, 0.3f, vulkanDevice, queue);
    ...
    if (vulkanDevice->features.textureCompressionBC) {
      ...
      textures.colorMap.loadFromFile(
          ...
          VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK, vulkanDevice, queue);
    } else {
      vks::tools::exitFatal(
...
#undef QUAD_COLOR_NORMAL
  ...
  void setupDescriptorSetLayout() {
    ...
    descriptorLayoutInfo = vks::initializers::descriptorSetLayoutCreateInfo(
        setLayoutBindings.data(), 1);
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        device, &descriptorLayoutInfo, nullptr, &descriptorSetLayouts.shaded));

    pipelineLayoutInfo = vks::initializers::pipelineLayoutCreateInfo(
        &descriptorSetLayouts.shaded, 1);
    ...
    pipelineLayoutInfo = vks::initializers::pipelineLayoutCreateInfo(
        &descriptorSetLayouts.textured, 1);
    VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr,
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorSetAllocateInfo allocInfo =
        vks::initializers::descriptorSetAllocateInfo(
            descriptorPool, &descriptorSetLayouts.textured, 1);

    VK_CHECK_RESULT(
        vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.mirror));

    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        // Binding 0 : Vertex shader uniform buffer
        vks::initializers::writeDescriptorSet(
            descriptorSets.mirror, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
            &uniformBuffers.vsMirror.descriptor),
        // Binding 1 : Fragment shader texture sampler
        vks::initializers::writeDescriptorSet(
            descriptorSets.mirror, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1,
            &offscreenPass.descriptor),
        // Binding 2 : Fragment shader texture sampler
        vks::initializers::writeDescriptorSet(
            descriptorSets.mirror, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2,
    ...
    std::vector<VkWriteDescriptorSet> debugQuadWriteDescriptorSets = {
        // Binding 0 : Vertex shader uniform buffer
        vks::initializers::writeDescriptorSet(
            descriptorSets.debugQuad, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
            &uniformBuffers.vsDebugQuad.descriptor),
        // Binding 1 : Fragment shader texture sampler
        vks::initializers::writeDescriptorSet(
            descriptorSets.debugQuad, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
            1, &offscreenPass.descriptor)};
    vkUpdateDescriptorSets(device, debugQuadWriteDescriptorSets.size(),
                           debugQuadWriteDescriptorSets.data(), 0, NULL);
    ...
    vkUpdateDescriptorSets(device, modelWriteDescriptorSets.size(),
                           modelWriteDescriptorSets.data(), 0, NULL);
    ...
    vkUpdateDescriptorSets(device, offScreenWriteDescriptorSets.size(),
                           offScreenWriteDescriptorSets.data(), 0, NULL);
  }
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/offscreen/quad.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/offscreen/quad.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);

    VkGraphicsPipelineCreateInfo pipelineCreateInfo =
        vks::initializers::pipelineCreateInfo(pipelineLayouts.textured,
                                              renderPass, 0);
    ...
    // Mirror
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/offscreen/mirror.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/offscreen/mirror.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    // Scene
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/offscreen/phong.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/offscreen/phong.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1,
                                              &pipelineCreateInfo, nullptr,
                                              &pipelines.shaded));
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.vsShared, sizeof(uboShared)));
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.vsMirror, sizeof(uboShared)));
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.vsOffScreen, sizeof(uboShared)));
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.vsDebugQuad, sizeof(uboShared)));
  ...
  void updateUniformBuffers() {
    // Mesh
    uboShared.projection = glm::perspective(
    ...
    uboShared.model = glm::rotate(uboShared.model, glm::radians(rotation.x),
                                  glm::vec3(1.0f, 0.0f, 0.0f));
    uboShared.model =
        glm::rotate(uboShared.model, glm::radians(rotation.y + meshRot.y),
                    glm::vec3(0.0f, 1.0f, 0.0f));
    uboShared.model = glm::rotate(uboShared.model, glm::radians(rotation.z),
    ...
    uboShared.model = glm::rotate(uboShared.model, glm::radians(rotation.x),
                                  glm::vec3(1.0f, 0.0f, 0.0f));
    uboShared.model = glm::rotate(uboShared.model, glm::radians(rotation.y),
                                  glm::vec3(0.0f, 1.0f, 0.0f));
    uboShared.model = glm::rotate(uboShared.model, glm::radians(rotation.z),
                                  glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    // Debug quad
    uboShared.projection = glm::ortho(
        4.0f, 0.0f, 0.0f, 4.0f * (float)height / (float)width, -1.0f, 1.0f);
    uboShared.model =
