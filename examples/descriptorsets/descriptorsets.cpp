...
public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      */
      for (auto cube : cubes) {
        // Bind the cube's descriptor set. This tells the command buffer to use
        // the uniform buffer and image set for this cube
        vkCmdBindDescriptorSets(drawCmdBuffers[i],
                                VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                                0, 1, &cube.descriptorSet, 0, nullptr);
  ...
  */
  void setupDescriptors() {
    /*
            ...
            The layout describes the shader bindings and types used for a
       certain descriptor layout and as such must match the shader bindings
    ...
    */
    ...
    /*
            ...
            Actual descriptors are allocated from a descriptor pool telling the
       driver what types and how many
            descriptors this application will use

            An application can have multiple pools (e.g. for multiple threads)
       with any number of descriptor types
            as long as device limits are not surpassed

            It's good practice to allocate pools with actually required
       descriptor types and counts
    ...
    */
    ...
    // Max. number of descriptor sets that can be allocted from this pool (one
    // per object)
    ...
    /*
            ...
            Using the shared descriptor set layout and the descriptor pool we
       will now allocate the descriptor sets.

            Descriptor sets contain the actual descriptor fo the objects
       (buffers, images) used at render time.

    */

    for (auto &cube : cubes) {

      // Allocates an empty descriptor set without actual descriptors from the
      // pool using the set layout
      ...
      VK_CHECK_RESULT(
          vkAllocateDescriptorSets(device, &allocateInfo, &cube.descriptorSet));
      ...
      */
      ...
      // once, even for multiple sets
      // This is possible because each VkWriteDescriptorSet also contains the
      // destination set to be updated
  ...
  void preparePipelines() {
    ...
    */
    ...
    // The pipeline layout is based on the descriptor set layout we created
    // above
    ...
    const std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages = {
        loadShader(getAssetPath() + "shaders/descriptorsets/cube.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT),
        loadShader(getAssetPath() + "shaders/descriptorsets/cube.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT)};

    pipelineCreateInfoCI.stageCount =
  ...
  void updateUniformBuffers() {
    ...
    for (auto &cube : cubes) {
      ...
      cube.matrices.model =
          glm::rotate(cube.matrices.model, glm::radians(cube.rotation.x),
                      glm::vec3(1.0f, 0.0f, 0.0f));
      cube.matrices.model =
          glm::rotate(cube.matrices.model, glm::radians(cube.rotation.y),
                      glm::vec3(0.0f, 1.0f, 0.0f));
      cube.matrices.model =
          glm::rotate(cube.matrices.model, glm::radians(cube.rotation.z),
