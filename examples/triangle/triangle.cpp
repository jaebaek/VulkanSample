...
*an displaying something
*	Contrary to the other examples, this one won't make use of helper
...
public:
  ...
  // requires to layout the graphics (and compute) pipeline states upfront
  // So for each combination of non-dynamic pipeline states you need a new
  // pipeline (there are a few exceptions to this not discussed here)
  // Even though this adds a new dimension of planing ahead, it's a great
  // opportunity for performance optimizations by the driver
  VkPipeline pipeline;
  ...
  // property flags we request (e.g. device local, host visibile)
  ...
  // buffers that are then resubmitted to the queue
  // This allows to generate work upfront and from multiple threads, one of the
  // biggest advantages of Vulkan
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      // frame buffer color attachment to
      // VK_IMAGE_LAYOUT_PRESENT_SRC_KHR for presenting it to the windowing
      // system
  ...
  void draw() {
    ...
    submitInfo.pCommandBuffers =
        &drawCmdBuffers[currentBuffer]; // Command buffers(s) to execute in this
                                        // batch (submission)
    ...
    // submit info as the wait semaphore for swap chain presentation
  ...
  void prepareVertices(bool useStagingBuffers) {
    ...
    if (useStagingBuffers) {
      ...
      // buffer for them
      // Note: Some devices offer a dedicated transfer queue (with only the
      // transfer bit set) that may be faster when doing lots of copies
  ...
  void setupDescriptorSetLayout() {
    ...
    // uniform buffers, image samplers, etc.
    ...
    // pipelines that are based on this descriptor set layout
    // In a more complex scenario you would have different pipeline layouts for
    // different descriptor set layouts that could be reused
  ...
  void setupDepthStencil() {
    ...
    // This allows for multiple views of one image with differing ranges (e.g.
  ...
  // and is a good opportunity to optimize especially on tile-based renderers
  // (with multiple subpasses)
  // Using sub pass dependencies also adds implicit layout transitions for the
  // attachment used, so we don't need to add explicit image memory barriers to
  // transform them
  // Note: Override of virtual function in the base class and called from within
  // VulkanExampleBase::prepare
  void setupRenderPass() {
    ...
    attachments[0].finalLayout =
        VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // Layout to which the attachment is
                                         ...
                                         // finished
    // As we want to present the color buffer to the swapchain, we transition to
    // PRESENT_KHR
    // Depth attachment
    ...
    subpassDescription.inputAttachmentCount = 0; // Input attachments can be
                                                 // used to sample from contents
                                                 // of a previous subpass
    ...
    // The actual usage layout is preserved through the layout specified in the
    // attachment reference
    // Each subpass dependency will introduce a memory and execution dependency
    // between the source and dest subpass described by
  ...
  void preparePipelines() {
    ...
    // states, replacing OpenGL's complex state machine
    // A pipeline is then stored and hashed on the GPU making pipeline changes
    // very fast
    // Note: There are still a few dynamic states that are not directly part of
    // the pipeline (but the info that they are used is)
    ...
    // dynamic states that can be changed within a command buffer
    // To be able to change these we need do specify which dynamic states will
    // be changed using this pipeline. Their actual states are set later on in
    // the command buffer.
    ...
    // operations
    // We only use depth tests and want depth tests and writes to be enabled and
    // compare with less or equal
  ...
  void prepareUniformBuffers() {
    ...
    // correct one to allocate memory from is crucial
    // We also want the buffer to be host coherent so we don't have to flush (or
    // sync after every update.
