...
*
* Summary:
* Use a device local buffer that stores draw commands for instanced rendering of
* different meshes stored
* in the same buffer.
*
* Indirect drawing offloads draw command generation and offers the ability to
* update them on the GPU
* without the CPU having to touch the buffer again, also reducing the number of
* drawcalls.
*
* The example shows how to setup and fill such a buffer on the CPU side, stages
...
public:
  ...
  // Store the indirect draw commands containing index offsets and instance
  // count per object
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.plants.vertices.buffer, offsets);
      // Binding point 1 : Instance data buffer
      vkCmdBindVertexBuffers(drawCmdBuffers[i], INSTANCE_BUFFER_BIND_ID, 1,
                             &instanceBuffer.buffer, offsets);

      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.plants.indices.buffer, 0,
      ...
      if (vulkanDevice->features.multiDrawIndirect) {
        vkCmdDrawIndexedIndirect(
            drawCmdBuffers[i], indirectCommandsBuffer.buffer, 0,
            indirectDrawCount, sizeof(VkDrawIndexedIndirectCommand));
      } else {
        // If multi draw is not available, we must issue separate draw commands
        for (auto j = 0; j < indirectCommands.size(); j++) {
          vkCmdDrawIndexedIndirect(drawCmdBuffers[i],
      ...
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.ground);
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &models.ground.vertices.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.ground.indices.buffer, 0,
                           VK_INDEX_TYPE_UINT32);
  ...
  void loadAssets() {
    models.plants.loadFromFile(getAssetPath() + "models/plants.dae",
                               vertexLayout, 0.0025f, vulkanDevice, queue);
    models.ground.loadFromFile(getAssetPath() + "models/plane_circle.dae",
                               vertexLayout, PLANT_RADIUS + 1.0f, vulkanDevice,
                               queue);
    models.skysphere.loadFromFile(getAssetPath() + "models/skysphere.dae",
                                  vertexLayout, 512.0f / 10.0f, vulkanDevice,
                                  queue);
    ...
    textures.plants.loadFromFile(getAssetPath() +
                                     "textures/texturearray_plants" +
                                     texFormatSuffix + ".ktx",
                                 texFormat, vulkanDevice, queue);
    textures.ground.loadFromFile(getAssetPath() + "textures/ground_dry" +
                                     texFormatSuffix + ".ktx",
                                 texFormat, vulkanDevice, queue);
  }
  ...
  void setupDescriptorSet() {
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        ...
        vks::initializers::writeDescriptorSet(descriptorSet,
                                              ...
                                              0, &uniformData.scene.descriptor),
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(
        getAssetPath() + "shaders/indirectdraw/indirectdraw.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() + "shaders/indirectdraw/indirectdraw.frag.spv",
    ...
    // Ground
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/indirectdraw/ground.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/indirectdraw/ground.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    // Skysphere
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/indirectdraw/skysphere.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/indirectdraw/skysphere.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareIndirectData() {
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &indirectCommandsBuffer,
        stagingBuffer.size));
  ...
  void prepareInstanceData() {
    ...
    for (uint32_t i = 0; i < objectCount; i++) {
      instanceData[i].rot =
          glm::vec3(0.0f, float(M_PI) * uniformDist(rndEngine), 0.0f);
      float theta = 2 * float(M_PI) * uniformDist(rndEngine);
      float phi = acos(1 - 2 * uniformDist(rndEngine));
      instanceData[i].pos =
          glm::vec3(sin(phi) * cos(theta), 0.0f, cos(phi)) * PLANT_RADIUS;
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &stagingBuffer, instanceData.size() * sizeof(InstanceData),
        instanceData.data()));
