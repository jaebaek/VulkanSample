...
public:
  ...
  void loadTextureArray(std::string filename, VkFormat format) {
    ...
    for (uint32_t layer = 0; layer < layerCount; layer++) {
      ...
      bufferCopyRegion.imageExtent.width =
          static_cast<uint32_t>(tex2DArray[layer][0].extent().x);
      bufferCopyRegion.imageExtent.height =
          static_cast<uint32_t>(tex2DArray[layer][0].extent().y);
    ...
    VK_CHECK_RESULT(
        vkCreateImage(device, &imageCreateInfo, nullptr, &textureArray.image));
    ...
    VK_CHECK_RESULT(vkAllocateMemory(device, &memAllocInfo, nullptr,
                                     &textureArray.deviceMemory));
    VK_CHECK_RESULT(vkBindImageMemory(device, textureArray.image,
                                      textureArray.deviceMemory, 0));
    ...
    vks::tools::setImageLayout(
        copyCmd, textureArray.image, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);

    // Copy the cube map faces from the staging buffer to the optimal tiled
    // image
    vkCmdCopyBufferToImage(copyCmd, stagingBuffer, textureArray.image,
    ...
    vks::tools::setImageLayout(copyCmd, textureArray.image,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                               textureArray.imageLayout, subresourceRange);
    ...
    VK_CHECK_RESULT(
        vkCreateSampler(device, &sampler, nullptr, &textureArray.sampler));
    ...
    VK_CHECK_RESULT(
        vkCreateImageView(device, &view, nullptr, &textureArray.view));
  ...
  void generateQuad() {
    ...
    std::vector<Vertex> vertices = {{{2.5f, 2.5f, 0.0f}, {1.0f, 1.0f}},
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorImageInfo textureDescriptor =
        vks::initializers::descriptorImageInfo(
            textureArray.sampler, textureArray.view, textureArray.imageLayout);
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/texturearray/instancing.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/texturearray/instancing.frag.spv",
  ...
  void prepareUniformBuffers() {
    ...
    uint32_t uboSize =
        sizeof(uboVS.matrices) + (layerCount * sizeof(UboInstanceData));
    ...
    for (int32_t i = 0; i < layerCount; i++) {
      ...
      uboVS.instance[i].model = glm::translate(
          glm::mat4(1.0f), glm::vec3(0.0f, i * offset - center, 0.0f));
      uboVS.instance[i].model =
          glm::rotate(uboVS.instance[i].model, glm::radians(60.0f),
    ...
    VK_CHECK_RESULT(vkMapMemory(device, uniformBufferVS.memory, dataOffset,
                                dataSize, 0, (void **)&pData));
  ...
  void updateUniformBufferMatrices() {
    ...
    // Projection
    uboVS.matrices.projection = glm::perspective(
        glm::radians(60.0f), (float)width / (float)height, 0.001f, 256.0f);

    // View
    uboVS.matrices.view =
        glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -1.0f, zoom));
    uboVS.matrices.view =
        glm::rotate(uboVS.matrices.view, glm::radians(rotation.x),
                    glm::vec3(1.0f, 0.0f, 0.0f));
    uboVS.matrices.view =
        glm::rotate(uboVS.matrices.view, glm::radians(rotation.y),
                    glm::vec3(0.0f, 1.0f, 0.0f));
    uboVS.matrices.view =
        glm::rotate(uboVS.matrices.view, glm::radians(rotation.z),
