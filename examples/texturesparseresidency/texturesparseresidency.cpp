...
struct VirtualTexturePage {
  ...
  VkSparseImageMemoryBind
      imageMemoryBind; // Sparse image memory bind for this page
  ...
  VirtualTexturePage() {
    imageMemoryBind.memory =
        VK_NULL_HANDLE; // Page initially not backed up by memory
  }
...
struct VirtualTexture {
  ...
  std::vector<VirtualTexturePage>
      pages; // Contains all virtual pages of the texture
  std::vector<VkSparseImageMemoryBind> sparseImageMemoryBinds; // Sparse image
                                                               // memory
                                                               // bindings of
                                                               // all
                                                               // memory-backed
                                                               // virtual tables
  std::vector<VkSparseMemoryBind> opaqueMemoryBinds; // Sparse ópaque memory
                                                     // bindings for the mip
                                                     // tail (if present)
  VkSparseImageMemoryBindInfo
      imageMemoryBindInfo; // Sparse image memory bind info
  VkSparseImageOpaqueMemoryBindInfo
      opaqueMemoryBindInfo; // Sparse image opaque memory bind info (mip tail)
  uint32_t mipTailStart;    // First mip level in mip tail

  VirtualTexturePage *addPage(VkOffset3D offset, VkExtent3D extent,
                              const VkDeviceSize size, const uint32_t mipLevel,
                              uint32_t layer) {
  ...
  void updateSparseBindInfo() {
    ...
    imageMemoryBindInfo.bindCount =
        static_cast<uint32_t>(sparseImageMemoryBinds.size());
...
public:
  ...
  glm::uvec3 alignedDivision(const VkExtent3D &extent,
                             const VkExtent3D &granularity) {
    glm::uvec3 res;
    res.x = extent.width / granularity.width +
            ((extent.width % granularity.width) ? 1u : 0u);
    res.y = extent.height / granularity.height +
            ((extent.height % granularity.height) ? 1u : 0u);
    res.z = extent.depth / granularity.depth +
            ((extent.depth % granularity.depth) ? 1u : 0u);
    return res;
  }

  void prepareSparseTexture(uint32_t width, uint32_t height,
    ...
    vkGetPhysicalDeviceSparseImageFormatProperties(
        ...
        &sparsePropertiesCount, nullptr);
    // Check if sparse is supported for this format
    if (sparsePropertiesCount == 0) {
      std::cout << "Error: Requested format does not support sparse features!"
                << std::endl;
      return;
    }
    ...
    vkGetPhysicalDeviceSparseImageFormatProperties(
        physicalDevice, format, VK_IMAGE_TYPE_2D, VK_SAMPLE_COUNT_1_BIT,
        VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_TILING_OPTIMAL,
        &sparsePropertiesCount, sparseProperties.data());

    std::cout << "Sparse image format properties: " << sparsePropertiesCount
              << std::endl;
    for (auto props : sparseProperties) {
      std::cout << "\t Image granularity: w = " << props.imageGranularity.width
                << " h = " << props.imageGranularity.height
    ...
    sparseImageCreateInfo.flags = VK_IMAGE_CREATE_SPARSE_BINDING_BIT |
    ...
    std::cout << "\t Alignment: " << sparseImageMemoryReqs.alignment
              << std::endl;

    // Check requested image size against hardware sparse limit
    if (sparseImageMemoryReqs.size >
        vulkanDevice->properties.limits.sparseAddressSpaceSize) {
      std::cout << "Error: Requested sparse image size exceeds supportes "
    ...
    vkGetImageSparseMemoryRequirements(
        device, texture.image, &sparseMemoryReqsCount, sparseMemoryReqs.data());
    if (sparseMemoryReqsCount == 0) {
      std::cout << "Error: No memory requirements for the sparse image!"
                << std::endl;
      return;
    }
    ...
    std::cout << "Sparse image memory requirements: " << sparseMemoryReqsCount
              << std::endl;
    for (auto reqs : sparseMemoryReqs) {
      std::cout << "\t Image granularity: w = "
                << reqs.formatProperties.imageGranularity.width
                << " h = " << reqs.formatProperties.imageGranularity.height
                << " d = " << reqs.formatProperties.imageGranularity.depth
                << std::endl;
      std::cout << "\t Mip tail first LOD: " << reqs.imageMipTailFirstLod
                << std::endl;
      std::cout << "\t Mip tail size: " << reqs.imageMipTailSize << std::endl;
      std::cout << "\t Mip tail offset: " << reqs.imageMipTailOffset
                << std::endl;
      std::cout << "\t Mip tail stride: " << reqs.imageMipTailStride
                << std::endl;
      // todo:multiple reqs
    ...
    if (!colorAspectFound) {
      std::cout << "Error: Could not find sparse image memory requirements for "
                   "color aspect bit!"
                << std::endl;
      return;
    }
    ...
    memoryTypeIndex =
        vulkanDevice->getMemoryType(sparseImageMemoryReqs.memoryTypeBits,
                                    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    // Get sparse bindings
    uint32_t sparseBindsCount = static_cast<uint32_t>(
        sparseImageMemoryReqs.size / sparseImageMemoryReqs.alignment);
    std::vector<VkSparseMemoryBind> sparseMemoryBinds(sparseBindsCount);

    // Check if the format has a single mip tail for all layers or one mip tail
    // for each layer
    // The mip tail contains all mip levels >
    // sparseMemoryReq.imageMipTailFirstLod
    bool singleMipTail = sparseMemoryReq.formatProperties.flags &
    ...
    for (uint32_t layer = 0; layer < texture.layerCount; layer++) {
      // sparseMemoryReq.imageMipTailFirstLod is the first mip level that's
      // stored inside the mip tail
      for (uint32_t mipLevel = 0;
           mipLevel < sparseMemoryReq.imageMipTailFirstLod; mipLevel++) {
        VkExtent3D extent;
        extent.width =
            std::max(sparseImageCreateInfo.extent.width >> mipLevel, 1u);
        extent.height =
            std::max(sparseImageCreateInfo.extent.height >> mipLevel, 1u);
        extent.depth =
            std::max(sparseImageCreateInfo.extent.depth >> mipLevel, 1u);
        ...
        VkExtent3D imageGranularity =
            sparseMemoryReq.formatProperties.imageGranularity;
        glm::uvec3 sparseBindCounts = alignedDivision(extent, imageGranularity);
        glm::uvec3 lastBlockExtent;
        lastBlockExtent.x = (extent.width % imageGranularity.width)
                                ? extent.width % imageGranularity.width
                                : imageGranularity.width;
        lastBlockExtent.y = (extent.height % imageGranularity.height)
                                ? extent.height % imageGranularity.height
                                : imageGranularity.height;
        lastBlockExtent.z = (extent.depth % imageGranularity.depth)
                                ? extent.depth % imageGranularity.depth
        ...
        for (uint32_t z = 0; z < sparseBindCounts.z; z++) {
          for (uint32_t y = 0; y < sparseBindCounts.y; y++) {
            for (uint32_t x = 0; x < sparseBindCounts.x; x++) {
              ...
              // Size of the page
              VkExtent3D extent;
              extent.width = (x == sparseBindCounts.x - 1)
                                 ? lastBlockExtent.x
                                 : imageGranularity.width;
              extent.height = (y == sparseBindCounts.y - 1)
                                  ? lastBlockExtent.y
                                  : imageGranularity.height;
              extent.depth = (z == sparseBindCounts.z - 1)
                                 ? lastBlockExtent.z
                                 : imageGranularity.depth;

              // Add new virtual page
              VirtualTexturePage *newPage = texture.addPage(
                  offset, extent, sparseImageMemoryReqs.alignment, mipLevel,
                  layer);
      ...
      if ((!singleMipTail) &&
        ...
        sparseMemoryBind.resourceOffset =
            sparseMemoryReq.imageMipTailOffset +
            layer * sparseMemoryReq.imageMipTailStride;
    ...
    // Fill image descriptor image info that can be used during the descriptor
    // set setup
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &heightMap->vertexBuffer.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], heightMap->indexBuffer.buffer, 0,
  ...
  void loadAssets() {
    textures.source.loadFromFile(
        getAssetPath() + "textures/ground_dry_bc3_unorm.ktx",
        VK_FORMAT_BC3_UNORM_BLOCK, vulkanDevice, queue,
        VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
  }

  // Generate a terrain quad patch for feeding to the tessellation control
  // shader
  void generateTerrain() {
    heightMap = new vks::HeightMap(vulkanDevice, queue);
#if defined(__ANDROID__)
    heightMap->loadFromFile(
        getAssetPath() + "textures/terrain_heightmap_r16.ktx", 128,
        glm::vec3(2.0f, 48.0f, 2.0f), vks::HeightMap::topologyTriangles,
  ...
  void preparePipelines() {
    ...
    shaderStages[0] = loadShader(
        getAssetPath() +
            "shaders/texturesparseresidency/sparseresidency.vert.spv",
        VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = loadShader(
        getAssetPath() +
            "shaders/texturesparseresidency/sparseresidency.frag.spv",
  ...
  void prepare() {
    ...
    // Create a virtual texture with max. possible dimension (does not take up
    // any VRAM yet)
  ...
  void fillVirtualTexture(int32_t &mipLevel) {
    ...
    for (auto &page : texture.pages) {
      if ((page.mipLevel == mipLevel) &&
          /*(rndDist(rndEngine) < 0.5f) &&*/ (page.imageMemoryBind.memory ==
                                              VK_NULL_HANDLE)) {
        ...
        for (uint32_t x = 0; x < scale; x++) {
          for (uint32_t y = 0; y < scale; y++) {
            ...
            blit.srcOffsets[1] = {static_cast<int32_t>(textures.source.width),
                                  static_cast<int32_t>(textures.source.height),
                                  1};
            // Dest
            ...
            blit.dstOffsets[0].x =
                static_cast<int32_t>(page.offset.x + x * 128 / scale);
            blit.dstOffsets[0].y =
                static_cast<int32_t>(page.offset.y + y * 128 / scale);
            blit.dstOffsets[0].z = 0;
            blit.dstOffsets[1].x = static_cast<int32_t>(
                blit.dstOffsets[0].x + page.extent.width / scale);
            blit.dstOffsets[1].y = static_cast<int32_t>(
                blit.dstOffsets[0].y + page.extent.height / scale);
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    ...
    if (overlay->header("Statistics")) {
      uint32_t respages = 0;
      std::for_each(
          texture.pages.begin(), texture.pages.end(),
          [&respages](VirtualTexturePage page) {
            respages += (page.imageMemoryBind.memory != VK_NULL_HANDLE) ? 1 : 0;
          });
      overlay->text("Resident pages: %d of %d", respages,
                    static_cast<uint32_t>(texture.pages.size()));
    }
  }
};
