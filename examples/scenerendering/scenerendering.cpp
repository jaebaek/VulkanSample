...
*
* The example loads a scene made up of multiple parts into one vertex and index
* buffer to only
* have one (big) memory allocation. In Vulkan it's advised to keep number of
* memory allocations
* down and try to allocate large blocks of memory at once instead of having many
* small allocations.
*
* Every part has a separate material and multiple descriptor sets (set = x
* layout qualifier in GLSL)
* are used to bind a uniform buffer with global matrices and the part's
* material's sampler at once.
*
* To demonstrate another way of passing data the example also uses push
* constants for passing
* material properties.
*
* Note that this example is just one way of rendering a scene made up of
* multiple parts in Vulkan.
*/
...
private:
  ...
  void loadMaterials() {
    ...
    for (size_t i = 0; i < materials.size(); i++) {
      ...
      materials[i].properties.ambient =
          glm::make_vec4(&color.r) + glm::vec4(0.1f);
      ...
      aScene->mMaterials[i]->Get(AI_MATKEY_OPACITY,
                                 materials[i].properties.opacity);
      ...
      if (aScene->mMaterials[i]->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
        std::cout << "  Diffuse: \"" << texturefile.C_Str() << "\""
                  << std::endl;
        std::string fileName = std::string(texturefile.C_Str());
        std::replace(fileName.begin(), fileName.end(), '\\', '/');
        fileName.insert(fileName.find(".ktx"), texFormatSuffix);
        materials[i].diffuse.loadFromFile(assetPath + fileName, texFormat,
                                          vulkanDevice, queue);
      } else {
        std::cout << "  Material has no diffuse, using dummy texture!"
                  << std::endl;
        // todo : separate pipeline and layout
        materials[i].diffuse.loadFromFile(assetPath + "dummy_rgba_unorm.ktx",
                                          VK_FORMAT_R8G8B8A8_UNORM,
                                          vulkanDevice, queue);
      }

      // For scenes with multiple textures per material we would need to check
      // for additional texture types, e.g.:
      // aiTextureType_HEIGHT, aiTextureType_OPACITY, aiTextureType_SPECULAR,
      // etc.

      // Assign pipeline
      materials[i].pipeline = (materials[i].properties.opacity == 0.0f)
                                  ? &pipelines.solid
                                  : &pipelines.blending;
    }
    ...
    VK_CHECK_RESULT(vkCreateDescriptorPool(vulkanDevice->logicalDevice,
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(vulkanDevice->logicalDevice,
    ...
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
        vulkanDevice->logicalDevice, &descriptorLayout, nullptr,
    ...
    // We will be using a push constant block to pass material properties to the
    // fragment shaders
    VkPushConstantRange pushConstantRange =
        vks::initializers::pushConstantRange(
            VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(SceneMaterialProperties), 0);
    pipelineLayoutCreateInfo.pushConstantRangeCount = 1;
    pipelineLayoutCreateInfo.pPushConstantRanges = &pushConstantRange;

    VK_CHECK_RESULT(vkCreatePipelineLayout(vulkanDevice->logicalDevice,
    ...
    for (size_t i = 0; i < materials.size(); i++) {
      ...
      VkDescriptorSetAllocateInfo allocInfo =
          vks::initializers::descriptorSetAllocateInfo(
              descriptorPool, &descriptorSetLayouts.material, 1);

      VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice,
                                               &allocInfo,
                                               &materials[i].descriptorSet));

      std::vector<VkWriteDescriptorSet> writeDescriptorSets;

      // todo : only use image sampler descriptor set and use one scene ubo for
      // matrices

      // Binding 0: Diffuse texture
      writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(
          materials[i].descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
          0, &materials[i].diffuse.descriptor));
    ...
    VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice,
                                             &allocInfo, &descriptorSetScene));

    std::vector<VkWriteDescriptorSet> writeDescriptorSets;
    // Binding 0 : Vertex shader uniform buffer
    writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(
        descriptorSetScene, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
  ...
  void loadMeshes(VkCommandBuffer copyCmd) {
    ...
    for (uint32_t i = 0; i < meshes.size(); i++) {
      ...
      std::cout << "	Material: \"" << materials[aMesh->mMaterialIndex].name
                << "\"" << std::endl;
      ...
      for (uint32_t v = 0; v < aMesh->mNumVertices; v++) {
        ...
        vertex.uv = hasUV ? glm::make_vec2(&aMesh->mTextureCoords[0][v].x)
                          : glm::vec2(0.0f);
        vertex.normal = hasNormals ? glm::make_vec3(&aMesh->mNormals[v].x)
                                   : glm::vec3(0.0f);
        vertex.normal.y = -vertex.normal.y;
        vertex.color = hasColor ? glm::make_vec3(&aMesh->mColors[0][v].r)
                                : glm::vec3(1.0f);
    ...
    // Create buffers
    // For better performance we only create one index and vertex buffer to keep
    // number of memory allocations down
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        &vertexStaging, static_cast<uint32_t>(vertexDataSize),
        vertices.data()));
    // Target
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &indexStaging, static_cast<uint32_t>(indexDataSize), indices.data()));
    // Target
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
...
public:
  ...
  Scene(vks::VulkanDevice *vulkanDevice, VkQueue queue) {
    ...
    VkBufferCreateInfo bufferCreateInfo = vks::initializers::bufferCreateInfo(
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(uniformData));
    VK_CHECK_RESULT(vkCreateBuffer(vulkanDevice->logicalDevice,
                                   &bufferCreateInfo, nullptr,
                                   &uniformBuffer.buffer));
    vkGetBufferMemoryRequirements(vulkanDevice->logicalDevice,
    ...
    memAlloc.memoryTypeIndex =
        vulkanDevice->getMemoryType(memReqs.memoryTypeBits,
                                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                        VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    VK_CHECK_RESULT(vkAllocateMemory(vulkanDevice->logicalDevice, &memAlloc,
                                     nullptr, &uniformBuffer.memory));
    VK_CHECK_RESULT(vkBindBufferMemory(vulkanDevice->logicalDevice,
                                       uniformBuffer.buffer,
                                       uniformBuffer.memory, 0));
    VK_CHECK_RESULT(vkMapMemory(vulkanDevice->logicalDevice,
                                uniformBuffer.memory, 0, sizeof(uniformData), 0,
  ...
  ~Scene() {
    ...
    vkDestroyPipelineLayout(vulkanDevice->logicalDevice, pipelineLayout,
                            nullptr);
    vkDestroyDescriptorSetLayout(vulkanDevice->logicalDevice,
                                 descriptorSetLayouts.material, nullptr);
    vkDestroyDescriptorSetLayout(vulkanDevice->logicalDevice,
                                 descriptorSetLayouts.scene, nullptr);
    vkDestroyDescriptorPool(vulkanDevice->logicalDevice, descriptorPool,
                            nullptr);
    ...
    vkDestroyPipeline(vulkanDevice->logicalDevice, pipelines.wireframe,
                      nullptr);
  ...
  void render(VkCommandBuffer cmdBuffer, bool wireframe) {
    ...
    for (size_t i = 0; i < meshes.size(); i++) {
      ...
      vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                        wireframe ? pipelines.wireframe
                                  : *meshes[i].material->pipeline);
      ...
      vkCmdPushConstants(
          cmdBuffer, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
          sizeof(SceneMaterialProperties), &meshes[i].material->properties);

      // Render from the global scene vertex buffer using the mesh index offset
      vkCmdDrawIndexed(cmdBuffer, meshes[i].indexCount, 1, 0,
                       meshes[i].indexBase, 0);
    }
  }
};
...
public:
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/scenerendering/scene.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/scenerendering/scene.frag.spv",
                   VK_SHADER_STAGE_FRAGMENT_BIT);

    VkGraphicsPipelineCreateInfo pipelineCreateInfo =
        vks::initializers::pipelineCreateInfo(scene->pipelineLayout, renderPass,
                                              0);
  ...
  void updateUniformBuffers() {
    ...
    memcpy(scene->uniformBuffer.mapped, &scene->uniformData,
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    if (overlay->header("Settings")) {
      ...
      if (scene) {
        ...
        if (overlay->checkBox("Render single part",
                              &scene->renderSingleScenePart)) {
          buildCommandBuffers();
        }
        if (scene->renderSingleScenePart) {
          if (overlay->sliderInt("Part to render", &scene->scenePartIndex, 0,
                                 static_cast<int32_t>(scene->meshes.size()))) {
            buildCommandBuffers();
          }
        }
      }
