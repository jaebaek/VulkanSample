...
*
* Push descriptors apply the push constants concept to descriptor sets. So
* instead of creating
* per-model descriptor sets (along with a pool for each descriptor type) for
* rendering multiple objects,
* this example uses push descriptors to pass descriptor sets for per-model
...
public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      for (auto cube : cubes) {
        ...
        // Instead of preparing the descriptor sets up-front, using push
        // descriptors we can set (push) them inside of a command buffer
        // This allows a more dynamic approach without the need to create
        // descriptor sets for each model
        // Note: dstSet for each descriptor set write is left at zero as this is
        // ignored when ushing push descriptors
  ...
  void setupDescriptorSetLayout() {
    ...
    // Setting this flag tells the descriptor set layouts that no actual
    // descriptor sets are allocated but instead pushed at command buffer
    // creation time
  ...
  void preparePipelines() {
    ...
    const std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages = {
        loadShader(getAssetPath() + "shaders/pushdescriptors/cube.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT),
        loadShader(getAssetPath() + "shaders/pushdescriptors/cube.frag.spv",
    ...
    for (auto &cube : cubes) {
      VK_CHECK_RESULT(
          vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                         VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                     &cube.uniformBuffer, sizeof(glm::mat4)));
  ...
  void updateCubeUniformBuffers() {
    ...
    for (auto &cube : cubes) {
      cube.modelMat = glm::rotate(cube.modelMat, glm::radians(cube.rotation.x),
                                  glm::vec3(1.0f, 0.0f, 0.0f));
      cube.modelMat = glm::rotate(cube.modelMat, glm::radians(cube.rotation.y),
                                  glm::vec3(0.0f, 1.0f, 0.0f));
      cube.modelMat = glm::rotate(cube.modelMat, glm::radians(cube.rotation.z),
  ...
  void prepare() {
    ...
    */
    ...
    // The push descriptor update function is part of an extension so it has to
    // be manually loaded
    vkCmdPushDescriptorSetKHR =
        (PFN_vkCmdPushDescriptorSetKHR)vkGetDeviceProcAddr(
            device, "vkCmdPushDescriptorSetKHR");
    if (!vkCmdPushDescriptorSetKHR) {
      vks::tools::exitFatal("Could not get a valid function pointer for "
                            "vkCmdPushDescriptorSetKHR",
                            -1);
    }
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
    ...
    if (overlay->header("Device properties")) {
      overlay->text("maxPushDescriptors: %d",
